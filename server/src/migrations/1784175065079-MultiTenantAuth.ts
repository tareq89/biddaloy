...
export class MultiTenantAuth1784175065079 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_user_tenants_user_tenant_role" ON "user_tenants" ("user_id", "tenant_id", "role") `,
    );
    await queryRunner.query(
    ...
    // 3. Add foreign keys
    await queryRunner.query(
      `ALTER TABLE "user_tenants" ADD CONSTRAINT "FK_user_tenants_user" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "user_tenants" ADD CONSTRAINT "FK_user_tenants_tenant" FOREIGN KEY ("tenant_id") REFERENCES "schools"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );

    // 4. Migrate existing users: create a default school and migrate all existing users
    const defaultSchoolId = await queryRunner.query(
      `INSERT INTO "schools" ("name", "slug") VALUES ('Default School', 'default-school') RETURNING "id"`,
    );
