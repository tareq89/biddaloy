...
export class AddPasswordResetAuditActions1788700000000 implements MigrationInterface {
  name = 'AddPasswordResetAuditActions1788700000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TYPE "public"."audit_logs_action_enum" ADD VALUE 'PASSWORD_RESET_REQUESTED'`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."audit_logs_action_enum" ADD VALUE 'PASSWORD_RESET'`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // Postgres has no `ALTER TYPE ... DROP VALUE` — recreate the type
    // without the added values. Fails loudly (by design) if any audit_logs
    // row already uses either: rolling back into data the old type can't
