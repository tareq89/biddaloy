...
function toSafeInvoice<T extends { issued_by: User | null }>(
  ...
  return { ...invoice, issued_by: invoice.issued_by ? UserResponseDto.fromEntity(invoice.issued_by) : null };
}
...
export class InvoicesController {
  ...
  @Get(':id')
  @Roles(UserRole.ADMIN, UserRole.ACCOUNTANT, UserRole.EXECUTIVE, UserRole.TEACHER)
  async findOne(@Param('id', ParseUUIDPipe) id: string, @CurrentTenant() tenant: { id: string; role: string }) {
