...
describe('resolveDetailMessage', () => {
  it('returns HttpException.message for a plain string response', () => {
    expect(resolveDetailMessage(new BadRequestException('Validation failed'))).toBe(
      'Validation failed',
    );
  });
  ...
  it('preserves an array-shaped message from getResponse(), where .message would collapse it', () => {
    ...
    expect(resolveDetailMessage(exception)).toEqual([
      'name should not be empty',
      'email must be an email',
    ]);
  });
...
describe('buildErrorResponseBody', () => {
  const opts = (nodeEnv: string | undefined) => ({
    path: '/api/things',
    requestId: 'req-1',
    nodeEnv,
  });
