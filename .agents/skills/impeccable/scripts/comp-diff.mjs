...
export function alignBuild(comp, build, align = 'top') {
  ...
  if (align === 'cover') {
    ...
    return crop(
      scaled,
      (scaled.width - comp.width) / 2,
      (scaled.height - comp.height) / 2,
      comp.width,
      comp.height,
    );
  }
  const scaled =
    build.width === comp.width
      ? build
      : resize(build, comp.width, Math.round((build.height / build.width) * comp.width));
...
export function scorePair(a, b, kind = null) {
  ...
  const bandsA = horizontalBands(a),
    bandsB = horizontalBands(b);
  const bands = bandScore(bandsA, bandsB);
  const w = WEIGHTS[kind] || WEIGHTS.default;
  const overall =
    w.structure * structure + w.color * color.score + w.detail * detail.score + w.bands * bands;
  return {
    overall: r4(overall),
...
export function verdictFor(s, kind = null) {
  ...
  if ((kind === 'chrome' || kind === 'control') && s.structure >= 0.5 && s.color >= 0.5)
    return s.overall >= 0.8 ? 'match' : 'drift';
...
export function resolveRegions(comp, spec) {
  ...
  if (spec && Array.isArray(spec.regions) && spec.regions.length) {
    for (const r of spec.regions) {
      ...
      regions.push({
        id: r.id || `region-${regions.length + 1}`,
        x: box.x,
        y: box.y,
        w: box.w,
        h: box.h,
...
export function inkBox(img) {
  ...
  const sample = [];
  for (let i = 0; i < g.data.length; i += Math.max(1, Math.floor(g.data.length / 4000)))
    sample.push(g.data[i]);
  sample.sort((p, q) => p - q);
  const ground = sample[Math.floor(sample.length / 2)] || 255;
  let x0 = img.width,
    y0 = img.height,
    x1 = -1,
    y1 = -1;
  for (let y = 0; y < img.height; y++)
    for (let x = 0; x < img.width; x++) {
      if (Math.abs(g.data[y * img.width + x] - ground) > 48) {
        if (x < x0) x0 = x;
        if (x > x1) x1 = x;
        if (y < y0) y0 = y;
        if (y > y1) y1 = y;
...
const HEAT_LABEL = {
  match: [40, 160, 80, 255],
  drift: [220, 160, 30, 255],
  missing: [200, 40, 40, 255],
  contradicted: [200, 40, 40, 255],
};
...
export function renderSideBySide(comp, build, label, score) {
  ...
  const a = fit(comp, targetW, 100000),
    b = resize(build, a.width, a.height);
  ...
  drawLabel(out, `BUILD ${label ? label.toUpperCase() : ''}`.trim(), pad + a.width + gap, pad - 4, {
    scale: 2,
  });
...
export function renderHeatmap(comp, build) {
  ...
  for (let i = 0, p = 0; i < map.data.length; i++, p += 4) {
    const d = map.data[i];
    if (d < 0.12) {
      // dim what matches so wrong stands out
      out.data[p] = out.data[p] * 0.55 + 255 * 0.45 * 0.2;
      out.data[p + 1] = out.data[p + 1] * 0.55;
      out.data[p + 2] = out.data[p + 2] * 0.55;
      continue;
    }
    const a = Math.min(1, (d - 0.12) / 0.5);
    out.data[p] = out.data[p] * (1 - a) + 235 * a;
    out.data[p + 1] = out.data[p + 1] * (1 - a) + 40 * a;
    out.data[p + 2] = out.data[p + 2] * (1 - a) + 40 * a;
  }
...
export function renderRegionPair(compCrop, buildCrop, id, score) {
  const gap = 16,
    pad = 12;
  const maxW = 700;
  const a = fit(compCrop, maxW, 700, true),
    b = resize(buildCrop, a.width, a.height);
  ...
  drawLabel(
    out,
    `BUILD  ${v.toUpperCase()} ${(score.overall * 100).toFixed(0)}%`,
    pad + a.width + gap,
    pad,
    { scale: 2, bg: HEAT_LABEL[v] },
  );
  return out;
}
...
export function compare({ comp, build, spec = null, align = 'top', label = '', kind = null }) {
  ...
  const regions = resolveRegions(comp, spec).map((r) => {
    const a = regionCrop(comp, r),
      b = regionCrop(aligned, r);
    const s = scorePair(a, b, r.kind);
    return {
      ...r,
      score: strip(s),
      verdict: verdictFor(s, r.kind),
      inkBox: { comp: inkBox(a), build: inkBox(b) },
      _a: a,
      _b: b,
    };
  });
  const compPalette = dominantColors(comp),
    buildPalette = dominantColors(aligned);
  return {
    label,
    align,
    whole: strip(whole),
...
export function writeArtifacts(result, comp, outDir) {
  ...
  fs.writeFileSync(
    ...
    encodePng(renderHeatmap(comp, result.aligned)),
  );
  const regionFiles = [];
  ...
  return {
    ...
    heatmap: path.join(outDir, 'heatmap.png'),
    regionFiles,
  };
}
...
export function buildReport(result, files, meta) {
  return {
    ...
    scores: result.whole,
    palette: {
      comp: result.compPalette.map(({ hex, coverage }) => ({ hex, coverage })),
      build: result.buildPalette.map(({ hex, coverage }) => ({ hex, coverage })),
    },
...
function summarize(report) {
  const lines = [];
  lines.push(
    `COMP-DIFF ${report.label ? `[${report.label}] ` : ''}overall ${(report.overall * 100).toFixed(0)}% (${report.verdict}) structure ${(report.scores.structure * 100).toFixed(0)}% color ${(report.scores.color * 100).toFixed(0)}% detail ${(report.scores.detail * 100).toFixed(0)}% bands ${(report.scores.bands * 100).toFixed(0)}%`,
  );
  lines.push(
    `PALETTE comp ${report.palette.comp
      .slice(0, 5)
      .map((c) => `${c.hex}(${Math.round(c.coverage * 100)}%)`)
      .join(' ')}`,
  );
  lines.push(
    `PALETTE build ${report.palette.build
      .slice(0, 5)
      .map((c) => `${c.hex}(${Math.round(c.coverage * 100)}%)`)
      .join(' ')}`,
  );
  for (const r of report.regions) {
    lines.push(
      `REGION ${r.id.padEnd(18)} ${r.verdict.padEnd(12)} ${(r.score.overall * 100).toFixed(0).padStart(3)}%  structure ${(r.score.structure * 100).toFixed(0).padStart(3)}%  color ${(r.score.color * 100).toFixed(0).padStart(3)}%  detail ${(r.score.detail * 100).toFixed(0).padStart(3)}%${r.score.detailAdded > 0.25 ? '  +invented detail' : ''}`,
    );
  }
  if (report.files) {
    ...
    lines.push(
      `FILES regions ${report.files.regionFiles.length} under ${path.dirname(report.files.regionFiles[0] || report.files.heatmap)}`,
    );
  }
  const worst = [...report.regions].sort((a, b) => a.score.overall - b.score.overall).slice(0, 3);
  if (worst.length)
    lines.push(
      `WORST ${worst.map((r) => `${r.id} (${r.verdict}, ${(r.score.overall * 100).toFixed(0)}%)`).join('; ')}`,
    );
  lines.push(
...
async function main() {
  ...
  let comp, build;
  try {
    comp = readPng(compPath);
  } catch (e) {
    console.error(`comp-diff: cannot read comp ${compPath}: ${e.message}`);
    process.exit(1);
  }
  try {
    build = readPng(buildPath);
  } catch (e) {
    console.error(`comp-diff: cannot read build ${buildPath}: ${e.message}`);
    process.exit(1);
  }
  let spec = null;
  ...
  if (specPath) {
    try {
      ...
      console.error(`comp-diff: cannot read spec ${specPath}: ${e.message}`);
      process.exit(1);
    }
  }
  ...
  const report = buildReport(result, files, {
    label,
    comp: compPath,
    build: buildPath,
    spec: specPath || null,
    compSize: `${comp.width}x${comp.height}`,
    buildSize: `${build.width}x${build.height}`,
  });
  ...
  if (threshold != null && report.overall < threshold) {
    if (!flag('json'))
      console.log(
        `BELOW THRESHOLD ${(threshold * 100).toFixed(0)}%: the reproduction is not done. Fix the worst regions and re-run; do not build past the hero.`,
      );
    process.exit(3);
  }
}
