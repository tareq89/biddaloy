...
export function withWeightVariants(list) {
  ...
  for (const c of list) {
    ...
    for (const d of [-200, 200]) {
      const nw = w + d;
      if (nw >= 100 && nw <= 900) out.push(`${m[1]}:${nw}`);
    }
  }
  return [...new Set(out)];
}
...
export function selectCandidates(fp, { own = [], index = null, n = 25, category = null } = {}) {
  ...
  if (!index) {
    source = 'shortlist';
    for (const s of withWeightVariants(SHORTLIST[widthClass(fp)] || SHORTLIST.normal))
      list.push(parseCandidates(s)[0]);
  }
  const seen = new Set();
  const candidates = list.filter((c) => {
    const k = `${c.family}:${c.weight}`;
...
export function stampChoice(regionId, chosen) {
  const h = createHash('sha1')
    .update(
      `font-match:${regionId}:${chosen.family}:${chosen.weight}:${chosen.fontSizePx}:${chosen.source}`,
    )
    .digest('hex')
    .slice(0, 12);
...
function ensureWritableTmp() {
  ...
  try {
    ...
    process.env.TMP = local;
    process.env.TEMP = local;
  } catch {
    /* leave as is; launch will say why */
  }
}
...
async function loadBrowser() {
  ...
  const tries = [
    ...extra.map(
      (dir) => () => require(require.resolve('playwright', { paths: [dir, path.dirname(dir)] })),
    ),
    () => require('playwright'),
  ...
  for (const t of tries) {
    try {
      const pw = t();
      if (pw?.chromium) return { kind: 'playwright', mod: pw };
    } catch {
      /* next */
    }
  }
  const tries2 = [
    ...extra.map(
      (dir) => () => require(require.resolve('puppeteer', { paths: [dir, path.dirname(dir)] })),
    ),
    () => require('puppeteer'),
  ...
  for (const t of tries2) {
    try {
      const pp = t();
      if (pp?.launch) return { kind: 'puppeteer', mod: pp };
    } catch {
      /* next */
    }
  }
...
export async function renderCandidates(candidates, text, targetCapPx, { transform = 'none' } = {}) {
  ...
  try {
    browser =
      b.kind === 'playwright'
        ? await b.mod.chromium.launch()
        : await b.mod.launch({ headless: true });
  } catch {
    return null;
  }
  ...
  const links = candidates
    .map(
      (c) =>
        `<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=${encodeURIComponent(c.family).replace(/%20/g, '+')}:wght@${c.weight}&display=block">`,
    )
    .join('');
    ...
    for (const c of candidates) {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        try {
          loaded = await page.evaluate(async (f) => {
            ...
            const covers = (face) => {
              ...
              const lo = w[0],
                hi = w[1] ?? w[0];
              return f.weight >= lo - 50 && f.weight <= hi + 50;
            };
            return faces.some(
              (face) =>
                face.family.replace(/["']/g, '') === f.family &&
                face.status === 'loaded' &&
                covers(face),
            );
          }, c);
        ...
        const box = await page.evaluate(() => {
          const r = document.querySelector('div.s').getBoundingClientRect();
          return { w: Math.ceil(r.width) + 8, h: Math.ceil(r.height) + 8 };
        });
        const buf = await page.screenshot({
          clip: { x: 0, y: 0, width: Math.min(1600, box.w), height: Math.min(400, box.h) },
        });
...
export async function renderProofSheet(compCrop, top, text, capPx, transform = 'none') {
  ...
  const rowsHtml = top
    .map(
      (c) =>
        `<div class="row"><div class="lab">${c.family} ${c.weight} · ${c.fontSizePx}px</div><div class="s" style="font-family:'${c.family}';font-weight:${c.weight};font-size:${c.fontSizePx}px;text-transform:${transform}">${text}</div></div>`,
    )
    .join('');
  ...
  let browser;
  try {
    browser = await b.mod.chromium.launch();
  } catch {
    return null;
  }
  const page = await browser.newPage({
    viewport: { width: Math.min(1600, Math.max(600, compCrop.width + 24)), height: 200 },
  });
  await page.setContent(html, { waitUntil: 'load' });
  try {
...
async function main() {
  ...
  if (!spec) {
    console.error(`font-match: no spec at ${specPath}; run comp-spec.mjs first`);
    process.exit(1);
  }
  const region = spec.regions.find((r) => r.id === id);
  if (!region) {
    console.error(`font-match: no region ${id}; ids: ${spec.regions.map((r) => r.id).join(', ')}`);
    process.exit(1);
  }
  ...
  if (!fp) {
    ...
    region.type = {
      ...
      note: 'no separable lettering in the crop; size by the region box',
    };
    fs.writeFileSync(specPath, JSON.stringify(spec, null, 2));
    console.log(
      `MEASURE ${id}: no separable lettering in the region crop at comp resolution; size this text by its box (${region.px.w}x${region.px.h}px) and inherit face and weight from the nearest measured region.`,
    );
    process.exit(0);
  }
  region.type = {
    ...(region.type || {}),
    comp: compactFp(fp),
    widthClass: widthClass(fp),
    weightClass: weightClass(fp),
  };
  fs.writeFileSync(specPath, JSON.stringify(spec, null, 2));
  console.log(
    `MEASURE ${id}: ${describe(fp)} over ${fp.lines} line${fp.lines === 1 ? '' : 's'}, ${fp.glyphs} glyphs. Set this region's font-size so its cap height renders at ${fp.capHeightPx}px; choose a ${widthClass(fp)} ${weightClass(fp)} face.`,
  );
  if (!rankId) return;
  if (fp.capHeightPx < MIN_RANK_CAP_PX) {
    console.log(
      `RANK skipped: cap height ${fp.capHeightPx}px is under ${MIN_RANK_CAP_PX}px, too small at comp resolution for a face fingerprint to mean anything. Size this text by its box (${region.px.w}x${region.px.h}px) and inherit face and weight from the nearest measured region.`,
    );
    return;
  }
  const own = parseCandidates(arg('candidates'));
  const index = loadFontIndex();
  const { candidates, catalog, source } = selectCandidates(fp, {
    own,
    index,
    n: 25,
  ...
  if (index) {
    ...
    for (const h of catalog) {
      if (!top5.some((t) => t.family === h.family)) top5.push(h);
      if (top5.length >= 5) break;
    }
    console.log(
      `CATALOG top-5 by fingerprint: ${top5.map((t) => `${t.family}:${t.weight}`).join(', ')} (from ${index.entries.length} indexed faces${catalog[0] ? `, ${catalog[0].size}px index` : ''}${arg('category') ? `, category ${arg('category')}` : ''})`,
    );
    console.log(
      `CANDIDATES ${candidates.length}: ${own.length} yours + ${candidates.length - own.length} nearest in the catalog index`,
    );
  } else {
    console.log(
      `CANDIDATES ${candidates.length}: ${own.length} yours + ${candidates.length - own.length} from the ${widthClass(fp)} shortlist (no catalog index at data/font-index.json)`,
    );
  }
  ...
  if (!results) {
    ...
    if (index && catalog[0]) {
      ...
      const fontSizePx = Math.round(fp.capHeightPx / 0.7);
      console.log(
        `RANK unavailable: no browser (playwright or puppeteer) resolvable from this project or the impeccable CLI; the CATALOG order stands as the ranking.`,
      );
      console.log(
        `USE font-family: '${best.family}'; font-weight: ${best.weight}; font-size: ${fontSizePx}px;${transform !== 'none' ? ` text-transform: ${transform};` : ''} NOTE font-size is estimated (cap ${fp.capHeightPx}px / 0.70); render one headline word at that size, compare its cap height to the comp crop, and correct the size before building on it.`,
      );
      region.type.chosen = stampChoice(id, {
        family: best.family,
        weight: best.weight,
        fontSizePx,
        source: 'catalog',
    ...
    console.log(
      `RANK unavailable: no browser (playwright or puppeteer) resolvable from this project or the impeccable CLI, and no catalog index. Choose by the MEASURE line: match the width class first, then the weight class; render one headline word against the comp before building on it.`,
    );
    return;
  }
  ...
  const rows = results
    ...
    .sort((a, b) => a.d - b.d)
    .filter((r) => {
      const k = `${r.family}|${r.fp.advX}|${r.fp.advTall}|${r.fp.densTall}|${r.fp.stemW}`;
  ...
  if (dropped.length)
    console.log(`SKIPPED (not available at that weight on Google Fonts): ${dropped.join(', ')}`);
  const wm = widthMeasure(fp),
    wt = weightMeasure(fp);
  const pctDelta = (m, other) => {
    if (!m || other?.[m.key] == null) return null;
    return (other[m.key] - m.value) / m.value;
  };
  const fmtPct = (v) => (v == null ? 'n/a' : `${v >= 0 ? '+' : ''}${(v * 100).toFixed(0)}%`);
  for (const r of rows) {
    console.log(
      `RANK ${r.family}:${r.weight} distance ${r.d.toFixed(3)}  width ${widthClass(r.fp)} (${fmtPct(pctDelta(wm, r.fp))} ${wm?.key || 'advance'})  weight ${weightClass(r.fp)} (${fmtPct(pctDelta(wt, r.fp))} ${wt?.key || 'ink'})  font-size ${r.fontSizePx}px for cap ${fp.capHeightPx}px`,
    );
  }
  ...
  try {
    ...
    if (sheet) {
      ...
      console.log(
        `PROOF ${out} (comp crop, then the top ${top.length} candidates at the comp's cap height; open it before choosing)`,
      );
    }
  } catch {
    /* proof sheet is best-effort */
  }
  const best = rows[0];
  if (best) {
    const advice = [];
    const dw = pctDelta(wm, best.fp),
      dwt = pctDelta(wt, best.fp);
    if (dw != null && Math.abs(dw) > 0.1)
      advice.push(
        dw > 0
          ? 'still too wide: try a more condensed face or a variable font with a wdth axis'
          : 'still too narrow: try a wider face',
      );
    ...
    if (dwt != null && Math.abs(dwt) > 0.15 && variable)
      advice.push(
        dwt > 0
          ? `too heavy: drop to weight ${Math.max(100, best.weight - 200)}`
          : `too light: raise to weight ${Math.min(900, best.weight + 200)}`,
      );
    console.log(
      `USE font-family: '${best.family}'; font-weight: ${best.weight}; font-size: ${best.fontSizePx}px;${transform !== 'none' ? ` text-transform: ${transform};` : ''}${advice.length ? ' NOTE ' + advice.join('; ') : ''}`,
    );
    region.type.chosen = stampChoice(id, {
      family: best.family,
      weight: best.weight,
...
if (isMain)
  main().catch((e) => {
    console.error(`font-match: ${e.message}`);
    process.exit(1);
  });
