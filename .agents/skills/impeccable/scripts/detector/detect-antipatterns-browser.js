...
(function () {
  ...
  function scanCssTextForOrganicClipPath(styleText) {
    ...
    while ((m = re.exec(styleText)) !== null) {
      ...
      if (kind === 'path') {
        ...
        findings.push({
          id: 'organic-clip-path',
          snippet: `clip-path: path() with ${curves} curve segments`,
          selector: enclosingCssSelector(styleText, m.index) || undefined,
        });
        continue;
      }
      const points = body
      ...
      for (const p of points) {
        ...
        for (const n of nums) {
          ...
          if (Math.abs(v - Math.round(v / 25) * 25) > 0.5) offGrid++;
        }
      }
      if (offGrid < points.length) continue;
      findings.push({
        id: 'organic-clip-path',
        snippet: `clip-path: polygon() with ${points.length} vertices approximating an organic contour`,
        selector: enclosingCssSelector(styleText, m.index) || undefined,
      });
    }
    return findings;
  ...
  function scanCssTextForBuriedRaster(styleText) {
    ...
    while ((m = declRe.exec(styleText)) !== null) {
      ...
      const rule = styleText.slice(
        ruleStart < 0 ? 0 : ruleStart,
        ruleEnd < 0 ? styleText.length : ruleEnd,
      );
      if (
        /background-blend-mode\s*:\s*(?!normal)/i.test(rule) ||
        /mix-blend-mode\s*:\s*(?!normal)/i.test(rule)
      )
        continue;
      ...
      const gradients = [
        ...value.matchAll(/(?:linear|radial|conic)-gradient\([^()]*(?:\([^()]*\)[^()]*)*\)/gi),
      ]
        .filter((gm) => gm.index < firstUrl)
        .map((gm) => gm[0]);
      ...
      const alphaOf = (a) => {
        ...
        return String(a).trim().endsWith('%') ? v / 100 : v;
      };
      for (const g of gradients) {
        const alphas = [
          ...g.matchAll(
            /rgba?\(\s*[\d.]+%?\s*,?\s*[\d.]+%?\s*,?\s*[\d.]+%?\s*(?:[,/]\s*([\d.]+%?))?\s*\)|hsla?\([^)]*?(?:[,/]\s*([\d.]+%?))?\s*\)/gi,
          ),
        ].map((a) => alphaOf(a[1] ?? a[2]));
        ...
        if (alphas.length && alphas.every((a) => !Number.isFinite(a) || a >= 0.9)) {
          opaqueWash = true;
          break;
        }
      }
      ...
      findings.push({
        id: 'buried-raster',
        snippet: `raster under a near-opaque gradient wash: ${value.trim().slice(0, 90)}`,
        selector: enclosingCssSelector(styleText, m.index) || undefined,
      });
    }
    return findings;
    ...
    if (elId.startsWith('claude-') || elId.startsWith('cic-')) return findings;
    {
      ...
      if (Number.isFinite(op) && op < 0.15 && op >= 0) {
        ...
        if (tag === 'img' || /url\(/i.test(bg)) {
          ...
          findings.push({
            id: 'buried-raster',
            snippet: `${tag === 'img' ? '<img>' : 'raster background'} at opacity ${op}${label ? ` "${label}"` : ''}`,
          });
        }
      }
    }
  ...
  function isRenderedTypeElement(el, getStyle) {
    for (let current = el; current; current = current.parentElement) {
      const hiddenAttr =
        typeof current.getAttribute === 'function' && current.getAttribute('hidden') !== null;
  ...
  `;
    ...
    function collectBrowserFindings() {
      ...
      const _disabledValues = EXTENSION_MODE
        ? (Array.isArray(window.__IMPECCABLE_CONFIG__?.disabledValues)
            ? window.__IMPECCABLE_CONFIG__.disabledValues
            : []
          )
      ...
      if (_disabledValues.length > 0) {
        ...
        const _valueIgnored = (f) => {
          ...
          return _disabledValues.some(
            (e) =>
              e.rule === rule &&
              (e.value === value ||
                (rule === 'design-system-color' &&
                  _colorKey(e.value) !== '' &&
                  _colorKey(e.value) === _colorKey(value))),
          );
        };
