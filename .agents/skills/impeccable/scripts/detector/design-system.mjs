...
function isMonorepoRoot(dir) {
  if (readWorkspacePatterns(dir).some((pattern) => !String(pattern).trim().startsWith('!')))
    return true;
  ...
  return MONOREPO_FALLBACK_PROJECT_DIRS.some((name) => {
    try {
      return fs
        .readdirSync(path.join(dir, name), { withFileTypes: true })
...
function monorepoOwnsPath(root, boundaryDir) {
  ...
  if (
    [...impeccable, ...pkg].some((pattern) => !normalizeWorkspacePattern(pattern).startsWith('!'))
  ) {
    return false;
  }
