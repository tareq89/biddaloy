...
export function compRoundOpen(cwd = process.cwd()) {
  ...
  if (fs.existsSync(pending) && !fs.existsSync(statePath))
    return {
      reason: 'a direction was chosen (concept-seed rolled) but build-phase.mjs start never ran',
      pending,
    };
  if (fs.existsSync(statePath)) {
    try {
      ...
      if (
        idx !== -1 &&
        idx <= PHASES.indexOf('hero') &&
        st.phases?.comps?.status !== 'skipped' &&
        st.phases?.comps?.status !== 'closed'
      )
        return {
          reason: `build-phase is at ${st.phase}; the comps phase never closed`,
          state: statePath,
        };
      if (idx !== -1 && idx <= PHASES.indexOf('hero'))
        return {
          reason: `build-phase is at ${st.phase}; the hero gate has not passed`,
          state: statePath,
        };
    } catch {
...
export function newState({ comp = null, breakpoint = null, artifact = null, direction = null }) {
  ...
  const phases = Object.fromEntries(
    PHASES.map((p) => [
      p,
      {
        status: p === first ? 'open' : 'pending',
        openedAt: p === first ? now() : null,
        closedAt: null,
        attempts: 0,
        notes: [],
        gate: null,
  ...
  if (comp) {
    phases.comps.status = 'skipped';
    phases.comps.notes.push({
      at: now(),
...
export function gateComps(state, { mocksDir = MOCKS_DIR } = {}) {
  ...
  if (comps.length < 3)
    reasons.push(
      `${comps.length} comp${comps.length === 1 ? '' : 's'} under ${mocksDir}; the comp round puts three compositional options of the chosen direction in front of the user (reference/visualize.md). Generate the missing ones (harness image tool or generate-image.mjs), each with a .json sidecar holding its prompt.`,
    );
  const noSidecar = comps.filter((c) => !c.sidecar);
  if (noSidecar.length)
    reasons.push(
      `no prompt sidecar for: ${noSidecar.map((c) => path.basename(c.file)).join(', ')} (write <file>.json with { "prompt": "..." }; generate-image.mjs does this itself)`,
    );
  const approved = comps.filter((c) => c.approved);
  if (approved.length === 0)
    reasons.push(
      'no comp is approved: put the three comps in front of the user (decision page via serve-question.mjs, or the structured question tool), then set "approved": true in the chosen comp\'s sidecar. A delegated pick is recorded the same way and disclosed.',
    );
  if (approved.length > 1)
    reasons.push(
      `${approved.length} comps carry "approved": true; exactly one is the approved comp: ${approved.map((c) => path.basename(c.file)).join(', ')}`,
    );
  return {
    ok: reasons.length === 0,
    reasons,
    summary: `${comps.length} comps, ${approved.length} approved`,
    approved: approved.length === 1 ? approved[0].file : null,
  };
}
...
export function gateSpec(state, { specPath = SPEC_PATH } = {}) {
  ...
  if (!spec)
    return {
      ok: false,
      reasons: [
        `no spec at ${specPath}: run comp-spec.mjs --comp ${state.comp} --grid, name the regions, then --regions regions.json`,
      ],
    };
  if (!spec.regions || spec.regions.length < 1)
    return { ok: false, reasons: ['spec has no regions'] };
  if (spec.comp && state.comp && path.resolve(spec.comp) !== path.resolve(state.comp)) {
    return {
      ok: false,
      reasons: [
        `spec measures ${spec.comp}, but this build started on ${state.comp}; re-run comp-spec on the approved comp`,
      ],
    };
  }
  ...
  if (cut.length)
    return {
      ...
      reasons: cut.map(
        (r) =>
          `region ${r.id}: the comp's artwork runs off its box on the ${r.clipped.join(' and ')}; widen the region's span so the box holds the whole shape with a margin (or set "bleed": true if the page really crops it there), then re-run comp-spec.mjs --regions`,
      ),
    };
  ...
  const textRegions = spec.regions
  ...
  if (textRegions.length) {
    ...
    if (!anyMeasured) {
      reasons.push(
        `measure the type before closing the spec: node ${HERE}/font-match.mjs --measure <id> for each text region (${textRegions.map((r) => r.id).join(', ')}); the region with the largest cap height is the lead and gets --rank.`,
      );
    ...
    if (!lead.type)
      reasons.push(
        `the lead text region ${lead.id} has no type measurement: run node ${HERE}/font-match.mjs --measure ${lead.id} (and --rank ${lead.id} --text "<its first words>" to choose the face by metrics). Set font-size from the printed cap height; do not pick a face by name.`,
      );
    else if (lead.type.comp && !lead.type.chosen)
      reasons.push(
        `the lead text region ${lead.id} is measured (${lead.type.widthClass} ${lead.type.weightClass}, cap ${lead.type.comp.capHeightPx}px) but no face is ranked: run node ${HERE}/font-match.mjs --rank ${lead.id} --text "<its first words>" [--candidates "Family:weight,..."] and use the USE line.`,
      );
    else if (lead.type.comp && lead.type.chosen && !choiceStamped(lead.id, lead.type.chosen))
      reasons.push(
        `the lead text region ${lead.id} carries a "chosen" face that font-match did not write (${lead.type.chosen.family || '?'}). A face typed into spec.json is the guess this gate exists to refuse; run node ${HERE}/font-match.mjs --rank ${lead.id} --text "<its first words>" and let it record the choice (with no browser it records the catalog's nearest face).`,
      );
    const unmeasured = textRegions
      .slice(1)
      .filter((r) => !r.type)
      .map((r) => r.id);
    if (unmeasured.length && !reasons.length)
      reasons.push(
        `measure the other text regions too, each sets its own font-size and weight class: node ${HERE}/font-match.mjs --measure <id> for ${unmeasured.join(', ')}`,
      );
  }
  ...
  return {
    ok: true,
    reasons: [],
    summary: `${spec.regions.length} regions, ${plates} plates, ${textRegions.length} text regions measured`,
  };
}
...
export function plateVerdict(region, score) {
  ...
  if (isTexture) {
    ...
    if (effective < PLATE_MIN)
      reasons.push(
        `scores ${(effective * 100).toFixed(0)}% as the material of region ${region.id} (color ${(score.color * 100).toFixed(0)}%, detail ${(score.detail * 100).toFixed(0)}%); crop a clean patch of the comp region (comp-spec.mjs --crop ${region.id} --raw) and mirror-tile it, generate only when no clean patch exists`,
      );
  ...
  if (compCalm && score.detailAdded > 0.45)
    reasons.push(
      `carries detail the comp region ${region.id} does not have (added-detail ${(score.detailAdded * 100).toFixed(0)}% of cells): noise, grain, or a busier subject where the comp is calm; regenerate from the crop reference without adding texture`,
    );
  if (score.structure < PLATE_STRUCTURE_MIN)
    reasons.push(
      `structure ${(score.structure * 100).toFixed(0)}% against the comp region ${region.id}: the composition of the plate is not the region's (different subject, orientation, or crop); regenerate with comp-spec.mjs --crop ${region.id} as the reference image`,
    );
  if (score.overall < PLATE_MIN)
    reasons.push(
      `scores ${(score.overall * 100).toFixed(0)}% against the comp region ${region.id} (structure ${(score.structure * 100).toFixed(0)}%, color ${(score.color * 100).toFixed(0)}%, detail ${(score.detail * 100).toFixed(0)}%); regenerate with the crop as --ref and the comp-spec plate prompt`,
    );
...
export function gatePlates(state, { specPath = SPEC_PATH } = {}) {
  ...
  if (!rasterRegions.length)
    return { ok: true, reasons: [], summary: 'no plates owed', plates: [] };
  let comp = null;
  try {
    comp = loadRaster(spec.comp).image;
  } catch {
    /* scored without the comp crop below */
  }
  const reasons = [],
    plates = [];
  for (const r of rasterRegions) {
    ...
    if (!file || !fs.existsSync(file)) {
      reasons.push(
        `plate missing for ${r.id}: expected ${file || '(no path)'}; produce it from comp-spec.mjs --crop ${r.id} with generate-image.mjs --plate`,
      );
      plates.push({ id: r.id, file, status: 'missing' });
      continue;
    }
    let img;
    try {
      ...
      plates.push({ id: r.id, file, status: 'unreadable' });
      continue;
    }
    ...
    if (!isTexture && img.width < minW)
      reasons.push(
        `plate ${file} is ${img.width}px wide; the comp region is ${r.px.w}px and a shipping plate needs at least ${Math.round(minW)}px. Regenerate at asset size, do not crop the comp.`,
      );
    let score = null;
    if (comp) {
      ...
      let build = img;
      let transparent = 0;
      for (let i = 3; i < img.data.length; i += 4) if (img.data[i] < 128) transparent++;
      ...
      if (!isTexture && !isFake) {
        ...
        if (same >= 0.95)
          reasons.push(
            `plate ${file} is the comp crop of region ${r.id} (structure ${(same * 100).toFixed(0)}% against the raw region, a resample of the same pixels): a crop of the comp is never a plate; generate the plate from the crop as reference (generate-image.mjs --plate ${r.id})`,
          );
      }
    }
    plates.push({
  ...
  return {
    ok: reasons.length === 0,
    reasons,
    summary: `${plates.filter((p) => p.status === 'ok').length}/${rasterRegions.length} plates`,
    plates,
  };
}
...
export function unreferencedPlates(spec, artifact = null) {
  ...
  if (artifact && fs.existsSync(artifact)) {
    ...
    try {
      ...
      for (const m of html.matchAll(/<link\b[^>]*href=["']([^"']+)["'][^>]*>/gi)) {
        ...
        if (clean.startsWith('/')) {
          linked.push(path.join(process.cwd(), clean), path.join(path.dirname(artifact), clean));
  ...
  for (const r of plates) {
    ...
    if (
      ...
      (corpus.includes('data:image/') && (corpus.includes(stem) || corpus.includes(r.id)))
    )
      continue;
    missing.push(r);
  }
...
export function organicClipRegions(artifactFile, spec) {
  ...
  let html = '';
  try {
    html = fs.readFileSync(artifactFile, 'utf8');
  } catch {
    return [];
  }
  ...
  for (const f of findings) {
    ...
    for (const r of rasterRegions) {
      ...
      if (
        (sel && (sel.includes(r.id.toLowerCase()) || (stem && sel.includes(stem)))) ||
        rasterRegions.length === 1
      ) {
        out.push({ id: r.id, snippet: f.snippet });
        break;
      }
    }
  }
...
export function writeScaffold(spec, state, { dir = path.join(BUILD_DIR, 'scaffold') } = {}) {
  ...
  const W = spec.compSize.width,
    H = spec.compSize.height;
  ...
  for (const r of spec.regions) {
    ...
    const b = r.box,
      id = r.id;
    vars.push(
      `  --r-${id}-x: ${pct(b.x)}; --r-${id}-y: ${pct(b.y)}; --r-${id}-w: ${pct(b.w)}; --r-${id}-h: ${pct(b.h)};`,
    );
    const type = r.type || {};
    const cap = type.comp && type.comp.capHeightPx;
    const chosen = type.chosen || null;
    const fontPx =
      chosen && chosen.fontSizePx ? chosen.fontSizePx : cap ? Math.round(cap / 0.7) : null;
    if (cap)
      vars.push(
        `  --r-${id}-cap: ${cap}px;${fontPx ? ` --r-${id}-font: ${fontPx}px;` : ''}${chosen ? ` --r-${id}-family: '${chosen.family}'; --r-${id}-weight: ${chosen.weight};` : ''}`,
      );
    if (chosen && chosen.family) fontLinks.add(`${chosen.family}:${chosen.weight}`);
    rules.push(
      `.r-${id} { position: absolute; left: var(--r-${id}-x); top: var(--r-${id}-y); width: var(--r-${id}-w); height: var(--r-${id}-h); }`,
    );
    const label = (r.note || id).replace(/</g, '&lt;');
    if (r.medium === 'raster' && r.kind !== 'texture') {
      const src = r.plate ? path.relative(dir, r.plate) : '';
      bodyParts.push(
        `  <figure class="r-${id} region plate" data-region="${id}"><img src="${src}" alt="" style="width:100%;height:100%;object-fit:contain;object-position:${r.kind === 'image' ? 'center' : 'top left'}"></figure>`,
      );
    } else if (r.kind === 'texture') {
      const src = r.plate ? path.relative(dir, r.plate) : '';
      bodyParts.push(
        `  <div class="r-${id} region texture" data-region="${id}" style="background-image:url('${src}');background-repeat:repeat"></div>`,
      );
    } else if (r.kind === 'text') {
      const style = [
        fontPx ? `font-size:var(--r-${id}-font)` : '',
        chosen
          ? `font-family:var(--r-${id}-family),sans-serif;font-weight:var(--r-${id}-weight)`
          : '',
        'line-height:1.05',
        'margin:0',
      ]
      ...
      bodyParts.push(
        `  <div class="r-${id} region text" data-region="${id}"><!-- ${label} --><p style="${style}">${(r.text || '').replace(/</g, '&lt;') || id}</p></div>`,
      );
    } else if (r.kind === 'control') {
      bodyParts.push(
        `  <div class="r-${id} region control" data-region="${id}"><!-- ${label}: rebuild the control's chrome from the crop (comp-spec.mjs --crop ${id}); its ink box, border, fill, radius, and label size are the comp's --></div>`,
      );
    } else {
      bodyParts.push(
        `  <div class="r-${id} region chrome" data-region="${id}"><!-- ${label} --></div>`,
      );
    }
  }
  vars.push('}');
  ...
  const link = fontLinks.size
    ? `  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?${[...fontLinks]
        .map((f) => {
          const [fam, w] = f.split(':');
...
export function heroReadings(state, spec, buildPath) {
  ...
  for (const r of spec.regions) {
    if (!r.px) continue;
    const a = crop(comp, r.px.x, r.px.y, r.px.w, r.px.h),
      b = crop(aligned, r.px.x, r.px.y, r.px.w, r.px.h);
    if (r.kind === 'text') {
      const t = textRegionCheck(r, a, b);
      text.push(...t.findings);
    } else if (r.kind === 'chrome' || r.kind === 'control') {
      const c = chromeStripCheck(r, a, b);
      chrome.push(...c.findings);
...
export function gateHero(
  state,
  {
    buildPath = HERO_REPRO,
    specPath = SPEC_PATH,
    min = HERO_MIN,
    outDir = path.join('.impeccable', 'review', 'diff', 'hero'),
    artifact = null,
  } = {},
) {
  if (!fs.existsSync(buildPath))
    return {
      ok: false,
      reasons: [
        `no hero capture at ${buildPath}: screenshot the first viewport at the comp's own dimensions (${state.breakpoint || 'comp size'}) into that path`,
      ],
    };
  ...
  if (res.status !== 0 && res.status !== 3)
    return { ok: false, reasons: [`comp-diff failed: ${res.stderr || res.stdout}`] };
  let report;
  try {
    report = JSON.parse(res.stdout);
  } catch {
    return { ok: false, reasons: ['comp-diff produced no report'] };
  }
  const reasons = [];
  const advisories = [];
  ...
  const [cw, ch] = String(report.compSize || '')
    .split('x')
    .map(Number);
  const [bw, bh] = String(report.buildSize || '')
    .split('x')
    .map(Number);
  if (cw && ch && bw && bh) {
    const compAspect = cw / ch,
      buildAspect = bw / bh;
    if (bw < cw * 0.9 || Math.abs(buildAspect - compAspect) / compAspect > 0.08)
      reasons.push(
        `hero capture is ${bw}x${bh}; the comp is ${cw}x${ch}. Capture the first viewport at the comp's own dimensions (viewport ${cw}x${ch}, not full page) into ${buildPath}.`,
      );
  }
  ...
  if (!aboveBar)
    reasons.push(
      `hero overall ${(report.overall * 100).toFixed(1)}% < ${(min * 100).toFixed(0)}% (structure ${(report.scores.structure * 100).toFixed(0)}%, color ${(report.scores.color * 100).toFixed(0)}%, detail ${(report.scores.detail * 100).toFixed(0)}%)`,
    );
  if (report.scores.colorIntersection != null && report.scores.colorIntersection < 0.2)
    reasons.push(
      `the palette is not the comp's (color intersection ${(report.scores.colorIntersection * 100).toFixed(0)}%): comp ${(
        report.palette.comp || []
      )
        .slice(0, 3)
        .map((c) => c.hex)
        .join(' ')} vs build ${(report.palette.build || [])
        .slice(0, 3)
        .map((c) => c.hex)
        .join(' ')}. Use the spec's sampled palette values, not a rendition of them.`,
    );
  ...
  const missing = report.regions.filter((r) => {
    ...
    if (!me) return true;
    const inkOver = specRegions.filter(
      (x) =>
        x.id !== r.id &&
        (x.kind === 'text' || x.kind === 'control' || x.kind === 'chrome') &&
        overlaps(me, x),
    );
    const inkPresent =
      inkOver.length > 0 && inkOver.every((x) => verdictOf[x.id] && verdictOf[x.id] !== 'missing');
    if (inkPresent) {
      r.verdict = 'drift';
      return false;
    }
  ...
  const passedPlate = (id) =>
    state.plates &&
    state.plates[id] &&
    state.plates[id].status === 'ok' &&
    (state.plates[id].score == null || state.plates[id].score >= PLATE_MIN);
  const placementNotes = [];
  for (const r of report.regions) {
    if (!(r.kind === 'plate' || r.kind === 'image' || r.kind === 'texture') || !passedPlate(r.id))
      continue;
    ...
    const present =
      r.kind === 'texture'
        ...
        : r.score.detailRaw != null
          ? r.score.detailRaw >= 0.3
          : r.score.detail >= 0.3;
    if (!present) continue; // nothing drawn there: still missing
    r.verdict = 'drift';
    r.placed = true;
    if (r.inkBox && r.inkBox.comp && r.inkBox.build) {
      const c = r.inkBox.comp,
        b = r.inkBox.build;
      const off =
        Math.abs(b.w - c.w) > c.w * 0.2 ||
        Math.abs(b.h - c.h) > c.h * 0.2 ||
        Math.abs(b.x - c.x) > c.w * 0.15 ||
        Math.abs(b.y - c.y) > c.h * 0.15;
      if (off)
        placementNotes.push(
          `plate ${r.id} is placed but not at the comp's box: its ink spans ${c.w}x${c.h}px at (${c.x},${c.y}) in the comp region and ${b.w}x${b.h}px at (${b.x},${b.y}) in the build; size and position the <img> to the spec box (object-fit: cover), not to the surrounding layout`,
        );
    }
  }
  const missingAfter = missing.filter((r) => r.verdict === 'missing');
  for (const r of missingAfter)
    reasons.push(
      `region ${r.id} is missing (detail ${(r.score.detail * 100).toFixed(0)}%, structure ${(r.score.structure * 100).toFixed(0)}%): the comp shows material the build does not`,
    );
  for (const n of placementNotes)
    (aboveBar ? advisories : reasons).push(
      aboveBar ? `(advisory, above the ${(min * 100).toFixed(0)}% bar) ${n}` : n,
    );
  ...
  const directionContradicted = contradicted.filter(
    (r) => r.kind === 'plate' || r.kind === 'image' || r.kind === 'text' || r.kind === 'control',
  );
  for (const r of directionContradicted)
    reasons.push(
      `region ${r.id} (${r.kind}) is contradicted (structure ${(r.score.structure * 100).toFixed(0)}%, detail added ${(r.score.detailAdded * 100).toFixed(0)}%): ${r.kind === 'text' ? 'the composition of this text region differs from the comp; re-derive it from the spec box' : r.kind === 'control' ? "this control does not read as the comp's: rebuild its chrome from the crop (border, fill, radius, chevron or arrow, label size) rather than from a component default" : 'the plate here does not read as the comp region; regenerate it with the crop as reference (generate-image.mjs --plate ' + r.id + ') and place it at its box'}`,
    );
  ...
  for (const r of report.regions) {
    ...
    reasons.push(
      `control ${r.id} drifts to ${(r.score.overall * 100).toFixed(0)}% (structure ${(r.score.structure * 100).toFixed(0)}%, color ${(r.score.color * 100).toFixed(0)}%): its chrome differs from the comp's; open ${path.join(outDir, 'regions', `${r.id}.png`)} and match the border, fill, radius, chevron or arrow, and label size`,
    );
  }
  ...
  for (const r of report.regions) {
    ...
    if (r.inkBox && r.inkBox.comp && r.inkBox.build) {
      ...
      const rwN = r.w ?? (r.box && r.box.w) ?? 1,
        rhN = r.h ?? (r.box && r.box.h) ?? 1;
      const rw =
        rwN * (report.compSize ? parseInt(String(report.compSize).split('x')[0], 10) : 1536);
      const rh =
        rhN * (report.compSize ? parseInt(String(report.compSize).split('x')[1], 10) : 1024);
      ...
      const dh = r.inkBox.build.h - r.inkBox.comp.h,
        dw = r.inkBox.build.w - r.inkBox.comp.w;
      // the build's box is only comparable when it is discrete too
      if (r.inkBox.build.w >= rw * 0.98 || r.inkBox.build.h >= rh * 0.98) continue;
      if (
        Math.abs(dh) > Math.max(6, r.inkBox.comp.h * 0.15) ||
        Math.abs(dw) > Math.max(12, r.inkBox.comp.w * 0.15)
      )
        (aboveBar ? advisories : reasons).push(
          `${aboveBar ? `(advisory, above the ${(min * 100).toFixed(0)}% bar) ` : ''}region ${r.id}: its ink sits in a ${r.inkBox.comp.w}x${r.inkBox.comp.h}px box in the comp and ${r.inkBox.build.w}x${r.inkBox.build.h}px in the build (padding, row height, or size); match the box, not only the position`,
        );
    }
  }
  const otherContradicted = contradicted.filter((r) => !directionContradicted.includes(r));
  if (otherContradicted.length > Math.max(1, Math.floor(report.regions.length / 3)))
    reasons.push(
      `${otherContradicted.length} of ${report.regions.length} regions contradicted: ${otherContradicted.map((r) => r.id).join(', ')}`,
    );
  ...
  if (artifactFile && fs.existsSync(artifactFile) && specForRefs) {
    ...
    for (const r of organic)
      reasons.push(
        `artifact draws an organic clip-path (${r.snippet}) inside raster region ${r.id}'s box; that region ships as its plate, never as a polygon`,
      );
    ...
    let svgs = [];
    try {
      svgs = svgIllustrations(fs.readFileSync(artifactFile, 'utf8'));
    } catch {
      svgs = [];
    }
    for (const v of svgs.slice(0, 6))
      reasons.push(
        `artifact draws an illustration in inline SVG${v.label ? ` (${v.label})` : ''}: ${v.snippet}. Drawings, diagrams, notation, and leader lines are plates or belong to the plate they annotate; only icon-sized SVG (under 64px, a few paths) is code`,
      );
  ...
  let readings = null;
  try {
    readings = heroReadings(state, specForRefs, buildPath);
  } catch (e) {
    reasons.push(`hero readings errored (${e.message}); the region scores above stand`);
  }
  if (readings) {
    ...
    const order = (f) =>
      /cap height/.test(f)
        ? 0
        : /lines? in the build/.test(f)
          ? 1
          : /heavier|lighter/.test(f)
            ? 2
            : /ink is/.test(f)
              ? 3
              : 4;
    ...
    for (const f of readings.text) {
      const m =
        /^text ([a-z0-9]+(?:-[a-z0-9]+)*?)(?:-(?:\d+|[a-z]))?: (cap height|\d+ lines? in the build|the face renders|ink is|its first line|it starts|line pitch)/i.exec(
          f,
        );
      const key = m ? `${m[1]}|${m[2].replace(/\d+/g, 'N')}` : f;
      if (!folded.has(key)) folded.set(key, { first: f, ids: [] });
      const idm = /^text ([^:]+):/.exec(f);
      if (idm) folded.get(key).ids.push(idm[1]);
    }
    const text = [...folded.values()]
      .map((v) => (v.ids.length > 1 ? `${v.first} (also ${v.ids.slice(1).join(', ')})` : v.first))
    ...
    if (aboveBar) {
      if (kept.length)
        advisories.push(
          `(advisory, above the ${(min * 100).toFixed(0)}% bar; fix in the polish pass before responsive) ${kept.length} reading${kept.length === 1 ? '' : 's'}:`,
        );
      for (const f of kept) advisories.push(`  ${f}`);
    } else {
      if (kept.length)
        reasons.push(
          `READINGS, each one CSS edit (${fresh.length > kept.length ? `${kept.length} of ${fresh.length}, the rest after these` : `${kept.length}`}):`,
        );
      for (const f of kept) reasons.push(f);
    }
    if (advisory.length)
      advisories.push(...advisory.map((f) => `(advisory, unchanged for 3+ attempts) ${f}`));
    ...
    const strongCells = (readings.invented ? readings.invented.cells : []).filter(
      (c) => c.build >= 22,
    );
    if (
      readings.invented &&
      (readings.invented.fraction >= INVENTED_MIN || strongCells.length >= 2)
    ) {
      const cells = readings.invented.cells.map((c) => c.label);
      reasons.push(
        `the build carries ink in ${cells.length} grid cells where the comp is calm (${cells.slice(0, 12).join(', ')}${cells.length > 12 ? ', ...' : ''}); nothing exists on the page that the comp does not show (a kicker, an extra nav item, a divider, a second row of controls); remove it or name it in a stated decision after the hero passes`,
      );
    }
  }
  const worstRegions = [...report.regions]
  ...
  return {
    ...
    worstCrops: worstRegions.map((r) => ({
      id: r.id,
      verdict: r.verdict,
      score: r.score,
      file: path.join(regionDir, `${r.id}.png`),
    })),
    advisories,
...
export function heroLoopVerdict(state, gate, artifactPath) {
  ...
  const entry = {
    ...
    worstIds: gate.worstIds || [],
    regionVerdicts: gate.regionVerdicts || {},
 ...
 */
export function gateResponsive(
  state,
  {
    specPath = SPEC_PATH,
    min = RESPONSIVE_MIN,
    outDir = path.join('.impeccable', 'review', 'diff', 'desktop'),
  } = {},
) {
  ...
  if (!fs.existsSync(desktop))
    reasons.push(
      `no ${desktop}: capture the page at a common desktop width (1440 wide, full page) into that path`,
    );
  if (!fs.existsSync(mobile))
    reasons.push(`no ${mobile}: capture the page at 390 wide, full page, into that path`);
  if (reasons.length) return { ok: false, reasons };
  const script = path.join(HERE, 'comp-diff.mjs');
  const args = [
    script,
  ...
  try {
    ...
    return { ok: false, reasons: [`comp-diff failed on ${desktop}: ${res.stderr || res.stdout}`] };
  }
...
export function runGate(state, phase, opts = {}) {
  ...
  try {
    ...
    return {
      ok: false,
      reasons: [
        `gate ${phase} errored (${e.message}); this is a tool bug, not a finding about the page. Re-run with the same inputs; if it repeats, note it and continue with build-phase.mjs advance --force --reason "user: gate ${phase} errored, proceeding" so the run is not lost.`,
      ],
...
export function forceAllowed(reason) {
  ...
  const isTranslationDodge =
    /truthful|semantic|pixel-level|prioriti[sz]e (facts|semantics|accessibility)/i.test(reason) &&
    !/(drop|skip|remove|without|not needed|don't need|do not need|ignore) (the )?(comp|plate|region|fidelity)/i.test(
      reason,
    );
  ...
  const downgrades =
    /\b(don't|do not|doesn't|does not|no longer|not) (need|have to|want|care|require|match|follow|hold)|\b(drop|skip|remove|ignore|waive|relax|override|approve|approved|accept|accepted|fine|okay|ok|good enough|ship it|move on|proceed|go ahead|instead of|rather than)\b/i.test(
      reason,
    );
  ...
  const reported =
    /["'\u201c\u2018].{6,}["'\u201d\u2019]|\b(user|they|paul) (said|says|asked|asks|told|wrote|replied|answered|chose|picked|approved|confirmed)\b/i.test(
      reason,
    );
  const briefQuoteOnly =
    /\b(should feel|feel like|not a .* page|extension of)\b/i.test(reason) &&
    !/\b(comp|mock|fidelity|gate|plate)\b.*\b(approved|accept|fine|ok|okay|skip|drop|waive|relax|override|move on|proceed)\b/i.test(
      reason,
    );
...
export function advance(state, { force = false, reason = null, gateOpts = {} } = {}) {
  ...
  if (idx === -1 || phase === 'review')
    return { ok: false, reasons: [`phase ${phase} cannot advance; use finish`] };
  ...
  if (phase === 'plates' && Array.isArray(gate.plates))
    state.plates = Object.fromEntries(
      gate.plates.map((pl) => [pl.id, { status: pl.status, score: pl.score, size: pl.size }]),
    );
  ...
  if (!gate.ok && !force) {
    p.status = 'open';
    return { ok: false, phase, reasons: gate.reasons, gate };
  }
  ...
  if (phase === 'comps' && gate.approved) {
    ...
    if (!state.breakpoint) {
      try {
        const i = loadRaster(gate.approved).image;
        state.breakpoint = `${i.width}x${i.height}`;
      } catch {
        /* non-png comp: breakpoint stays unset */
      }
    }
  }
  const next = PHASES[idx + 1];
  state.phase = next;
  state.phases[next].status = 'open';
  state.phases[next].openedAt = now();
...
export function renderStatus(state) {
  const lines = [
    `BUILD-PHASE ${state.phase.toUpperCase()}  comp ${state.comp || '(pending comp round)'}${state.direction ? `  direction ${state.direction}` : ''}${state.breakpoint ? `  breakpoint ${state.breakpoint}` : ''}`,
  ];
...
async function main() {
  ...
  if (cmd === 'start') {
    ...
    if (comp && !fs.existsSync(comp)) {
      console.error(`build-phase: comp ${comp} does not exist`);
      process.exit(1);
    }
    ...
    if (direction && arg('kind')) {
      try {
        ...
        console.log('choice ping skipped');
      }
    }
    // Clear the roll's pending marker: the build has started.
    try {
      fs.rmSync(path.join(BUILD_DIR, 'pending.json'), { force: true });
    } catch {
      /* absent */
    }
    ...
    if (direction && !comp && buildPath === 'code') {
      console.log(
        'CODE-LED (from .impeccable config): no comp round and no phase gates. Write the direction contract (reference/new-work.md section 5), build, and finish per section 7. The chosen decision comp, if any, rides to the finish review as the critique reference.',
      );
      return;
    }
    ...
    if (!breakpoint && comp) {
      try {
        const i = loadRaster(comp).image;
        breakpoint = `${i.width}x${i.height}`;
      } catch {
        /* leave null */
      }
    }
    ...
    if (existing && !flag('reset')) {
      console.log(
        `build-phase: state exists (phase ${existing.phase}); pass --reset to start over`,
      );
  ...
  if (!state) {
    console.error(
      `build-phase: no state at ${STATE_PATH}; run build-phase.mjs start --comp <approved comp>`,
    );
    process.exit(1);
  }
  if (cmd === 'status') {
  ...
  if (cmd === 'scaffold') {
    const spec = loadSpec();
    if (!spec) {
      console.error(`build-phase: no spec at ${SPEC_PATH}; run comp-spec.mjs first`);
      process.exit(1);
    }
    const out = writeScaffold(spec, state);
    console.log(`SCAFFOLD ${out.dir}`);
    console.log(
      `  ${out.css}   one custom property set per region (--r-<id>-x/y/w/h in % of the comp; --r-<id>-cap, --r-<id>-font, --r-<id>-weight where measured); bind these to your own markup`,
    );
    console.log(
      `  ${out.html}  a reference page: every region positioned at its box inside a ${state.breakpoint || spec.compSize.width + 'x' + spec.compSize.height} frame, plates placed with object-fit: contain, text slots at the measured cap height in the ranked face`,
    );
    console.log(
      '  The reference is a check, not the page: keep your own semantic structure and bind the numbers to it (an element per region, its box from the properties). Overlapping boxes are overlapping boxes. What the gate reads is pixels; a page that lands each region at its box passes whatever markup it uses.',
    );
    return;
  }
  if (cmd === 'note') {
  ...
  if (cmd === 'record') {
    ...
    if (which !== 'hero') {
      console.error('build-phase: record hero --build <png>');
      process.exit(1);
    }
    const gate = gateHero(state, {
      buildPath: arg('build', HERO_REPRO),
      min: arg('min') ? parseFloat(arg('min')) : HERO_MIN,
    });
    ...
    const plateRows = Object.entries(gate.regionVerdicts || {}).filter(([id]) => {
      const spec = loadSpec();
      const r = spec && spec.regions.find((x) => x.id === id);
      return r && r.medium === 'raster';
    });
    if (plateRows.length)
      console.log(`PLATES ${plateRows.map(([id, v]) => `${id}:${v}`).join(' ')}`);
  ...
  if (cmd === 'advance') {
    ...
    if (!res.ok) {
      ...
      if (res.gate && res.gate.worstCrops && res.gate.worstCrops.length) {
        console.log(
          '  LOOK FIRST, in this order, before editing anything (comp on the left, your build on the right):',
        );
        for (const c of res.gate.worstCrops)
          console.log(
            `    ${c.file}   ${c.id}: ${c.verdict} ${(c.score.overall * 100).toFixed(0)}% (structure ${(c.score.structure * 100).toFixed(0)}%, color ${(c.score.color * 100).toFixed(0)}%, detail ${(c.score.detail * 100).toFixed(0)}%)`,
          );
        console.log(
          '  A region scored missing needs its material (a plate placed, or produced), not a value change; contradicted needs its structure re-derived from the spec box; drift is where padding and size edits belong. When a thin chrome strip (masthead, breadcrumb, table header) is the worst region, check its box height in the spec against the comp first: a strip one grid row tall in the spec but 53px in the comp compares your build against ground it never had.',
        );
      }
      for (const r of res.reasons) console.log(`  - ${r}`);
      if (res.gate && res.gate.advisories && res.gate.advisories.length)
        for (const a of res.gate.advisories) console.log(`  ${a}`);
      if (res.gate && res.gate.sideBySide)
        console.log(`  then ${res.gate.sideBySide} for the whole viewport`);
      process.exit(2);
    }
    console.log(
      `ADVANCED ${res.phase} -> ${res.next}${res.forced ? ' (FORCED; recorded)' : ''}${res.gate.summary ? `  ${res.gate.summary}` : ''}`,
    );
    if (res.gate && res.gate.advisories && res.gate.advisories.length)
      for (const a of res.gate.advisories) console.log(`  ${a}`);
  ...
  if (cmd === 'finish') {
    ...
    if (!['ship', 'fix', 'rebuild', 'recapture'].includes(disposition)) {
      console.error('build-phase: finish --disposition ship|fix|rebuild|recapture');
      process.exit(1);
    }
    ...
    const openBefore = PHASES.filter(
      (ph) =>
        ph !== 'review' &&
        state.phases[ph] &&
        state.phases[ph].status !== 'closed' &&
        state.phases[ph].status !== 'skipped',
    );
    if (disposition === 'ship' && openBefore.length) {
      console.error(
        `build-phase: finish --disposition ship refused: ${openBefore.join(', ')} ${openBefore.length === 1 ? 'is' : 'are'} not closed (phase ${state.phase}). Record fix or rebuild, or close the phases first; a page shipped over an open hero is a page shipped against its own gate.`,
      );
      process.exit(2);
    }
    state.finish = { disposition, at: now(), phaseAtFinish: state.phase };
    if (state.phase === 'review') {
      state.phases.review.status = 'closed';
      state.phases.review.closedAt = now();
    }
    saveState(state);
    console.log(renderStatus(state));
    return;
