...
function addIgnoreValue(cwd, args) {
  ...
  if (
    parsed.value !== '*' &&
    !extractFindingIgnoreValue({ antipattern: parsed.rule, ignoreValue: parsed.value })
  ) {
    throw new Error(
      `${parsed.rule} has no extractable ignore value. Use ${IMPECCABLE_COMMAND} hooks ignore-value ${parsed.rule} "*" --file <glob> to suppress it in matching files.`,
    );
  }
...
function reset(cwd) {
  ...
  const parts = [];
  if (removed.length)
    parts.push(`Reset design hook config and cache (removed: ${removed.join(', ')}).`);
