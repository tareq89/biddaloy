...
function snapshotTargetIdentity(snapshot) {
  ...
  return snapshot?.meta.target_identity || (targetPath ? `file:${targetPath}` : null);
}
...
function readNewestSnapshotForIdentity(slug, targetIdentity, { cwd = process.cwd() } = {}) {
...
export function closeSnapshot(snapshotFile, { cwd = process.cwd() } = {}) {
  ...
  if (path.dirname(snapshotPath) !== dir || !SNAPSHOT_FILENAME.test(filename)) return null;
  ...
  const closedBody = snapshot.body.replace(/^(---\r?\n[\s\S]*?)(\r?\n---)/, '$1\nclosed: true$2');
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'close': {
      ...
      if (recordedTargetIdentity && recordedTargetIdentity !== resolveTargetIdentity(slugArg))
        process.exit(2);
