...
async function appendCompRoundOpenDirective(parts, ctx) {
  try {
    ...
    for (const root of roots) {
      ...
      if (!open) continue;
      parts.push(
        `COMP_ROUND_OPEN: ${open.reason}. On a comp-led build no page code is written before build-phase.mjs closes the comps, spec, plates, and hero gates; run \`node ${path.dirname(fileURLToPath(import.meta.url))}/build-phase.mjs status\` and follow its NEXT line. A page written past an open round is what the finish reviewer sends back.`,
      );
      return;
    }
  } catch {
    /* build-phase absent: nothing to say */
  }
}
