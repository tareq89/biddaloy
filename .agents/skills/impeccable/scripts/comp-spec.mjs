...
export function gridToBox(span) {
  ...
  const c0 = COLS.indexOf(m[1].toUpperCase()),
    r0 = +m[2],
    c1 = COLS.indexOf(m[3].toUpperCase()),
    r1 = +m[4];
...
export function renderGrid(comp) {
  ...
  for (let r = 0; r < 10; r++)
    for (let c = 0; c < 10; c++) {
      drawLabel(img, `${COLS[c]}${r}`, Math.round(c * cw) + 3, Math.round(r * ch) + 3, {
        scale: 2,
        bg: [0, 0, 0, 170],
        fg: [255, 230, 120, 255],
      });
 ...
 */
export function artworkTouchesEdges(
  img,
  { contact = EDGE_CONTACT_MIN, band = 2, ground = null } = {},
) {
  const W = img.width,
    H = img.height;
  ...
  if (ground == null) {
    const sample = [];
    for (let i = 0; i < gray.length; i += Math.max(1, Math.floor(gray.length / 5000)))
      sample.push(gray[i]);
    sample.sort((a, b) => a - b);
    ground = sample[Math.floor(sample.length / 2)];
  }
  ...
  if (
    run(H, (y) => {
      for (let x = 0; x < band; x++) if (ink(x, y)) return true;
      return false;
    }) >= contact
  )
    sides.push('left');
  if (
    run(H, (y) => {
      for (let x = W - band; x < W; x++) if (ink(x, y)) return true;
      return false;
    }) >= contact
  )
    sides.push('right');
  if (
    run(W, (x) => {
      for (let y = 0; y < band; y++) if (ink(x, y)) return true;
      return false;
    }) >= contact
  )
    sides.push('top');
  if (
    run(W, (x) => {
      for (let y = H - band; y < H; y++) if (ink(x, y)) return true;
      return false;
    }) >= contact
  )
    sides.push('bottom');
...
export function snapBoxToInk(comp, box, ground, { pad = 6, minShrink = 0.06 } = {}) {
  const px = {
    x: Math.round(box.x * comp.width),
    y: Math.round(box.y * comp.height),
    w: Math.round(box.w * comp.width),
    h: Math.round(box.h * comp.height),
  };
    ...
    for (let x = 0; x < cw; x++) {
      ...
      for (let dy = -1; dy <= 1; dy++)
        for (let dx = -1; dx <= 1; dx++) {
          const nx = x + dx,
            ny = y + dy;
          if (nx >= 0 && ny >= 0 && nx < cw && ny < ch) grown[ny * cw + nx] = 1;
        }
    }
  const mask = grown;
  ...
  for (let s0 = 0; s0 < on.length; s0++) {
    ...
    else if (!best.touchesSide && cand.touchesSide && cand.n < best.n * 3) {
      /* keep inside */
  ...
  if (best) {
    x0 = best.bx0 * cell;
    y0 = best.by0 * cell;
    x1 = Math.min(W - 1, (best.bx1 + 1) * cell - 1);
    y1 = Math.min(H - 1, (best.by1 + 1) * cell - 1);
  }
  const nx0 = Math.max(0, x0 - pad),
    ny0 = Math.max(0, y0 - pad),
    nx1 = Math.min(W, x1 + 1 + pad),
    ny1 = Math.min(H, y1 + 1 + pad);
  const shrink = 1 - ((nx1 - nx0) * (ny1 - ny0)) / (W * H);
  if (shrink < minShrink) return null;
  return {
    x: (px.x + nx0) / comp.width,
    y: (px.y + ny0) / comp.height,
    w: (nx1 - nx0) / comp.width,
    h: (ny1 - ny0) / comp.height,
  };
}
...
function medianGray(img) {
  ...
  for (let j = 0; j < img.width * img.height; j += step) {
    const i = j * 4;
    sample.push(0.299 * img.data[i] + 0.587 * img.data[i + 1] + 0.114 * img.data[i + 2]);
  }
  sample.sort((a, b) => a - b);
...
export function uncoveredInkCells(comp, regions) {
  ...
  for (let r = 0; r < 10; r++)
    for (let c = 0; c < 10; c++) {
      ...
      const cx = (c + 0.5) / 10,
        cy = (r + 0.5) / 10;
      const covered = regions.some((reg) => {
        const b = reg.coverBox || reg.box;
        return (
          reg.kind !== 'texture' &&
          reg.kind !== 'band' &&
          cx >= b.x &&
...
export function measureRegions(comp, regionsInput, compPath) {
  ...
  for (const raw of regionsInput.regions || []) {
    ...
    if (kind !== 'band' && !(raw.note && String(raw.note).trim().length >= 8)) {
      throw new Error(
        `region ${raw.id} has no note. Say in a few words what the comp shows there (the element, its material, its role): the note drives the plate prompt and the gate's messages, and a drawing named as chrome is only caught by what its note says.`,
      );
    }
    ...
    for (const key of ['codeDrawn', 'container', 'bleed']) {
      if (raw[key])
        warnings.push(
          `region ${raw.id}: "${key}": true set in the regions file${key === 'codeDrawn' ? ' (the painted-material refusal is overridden: code draws this region)' : key === 'container' ? ' (the region-size refusal is overridden: one undivided element)' : ' (the clipped-artwork refusal is overridden: the page crops it there)'}`,
        );
    }
    if (
      raw.note &&
      !RASTER_KINDS.has(kind) &&
      kind !== 'band' &&
      PAINTED_NOTE.test(raw.note) &&
      !raw.codeDrawn
    ) {
      throw new Error(
    ...
    const atCompEdge = {
      left: px.x <= 1,
      top: px.y <= 1,
      right: px.x + px.w >= comp.width - 1,
      bottom: px.y + px.h >= comp.height - 1,
    };
    const clipped =
      raster && kind !== 'texture' && !raw.bleed
        ? artworkTouchesEdges(c, { ground: pageGround }).filter((side) => !atCompEdge[side])
        : [];
    if (clipped.length)
      warnings.push(
        `region ${raw.id}: the artwork runs off the box on the ${clipped.join(' and ')} (its ink reaches the edge over ${EDGE_CONTACT_MIN * 100}% of that side). Widen the region so the box holds the whole shape with a margin; a plate placed with object-fit: cover on this box would be cut there.`,
      );
    regions.push({
      id: raw.id,
      kind,
      ...
      coverBox: coverBox
        ? { x: r4(coverBox.x), y: r4(coverBox.y), w: r4(coverBox.w), h: r4(coverBox.h) }
        : undefined,
  ...
  if (uncovered.length > 3 && !regionsInput.allowUncovered) {
    throw new Error(
      `grid cells ${uncovered.join(', ')} carry ink no region names. Every element the comp shows must be in a region (text, control, chrome, or a plate) so its absence in the build can be measured; add regions for them, or set "allowUncovered": true in the regions file after confirming those cells are empty ground.`,
    );
  }
  return {
    tool: 'comp-spec',
    ...
    bands: horizontalBands(comp)
      .filter((b) => b.strength > 0.2)
      .map((b) => ({ y: r4(b.y), strength: r4(b.strength) })),
    regions,
  };
}
...
export function autoRegions(comp) {
  ...
  const cuts = [0, ...bands.map((b) => b.y), 1].filter(
    (v, i, arr) => i === 0 || v - arr[i - 1] > 0.06,
  );
...
export function plateReference(comp, spec, region) {
  ...
  const ground = (region.palette && region.palette[0] && hexToRgb(region.palette[0].hex)) || [
    255, 255, 255,
  ];
  for (const other of spec.regions || []) {
    if (other.id === region.id || RASTER_KINDS.has(other.kind) || other.kind === 'band') continue;
    const ox = Math.max(0, other.px.x - region.px.x),
      oy = Math.max(0, other.px.y - region.px.y);
    const ox2 = Math.min(region.px.w, other.px.x + other.px.w - region.px.x),
      oy2 = Math.min(region.px.h, other.px.y + other.px.h - region.px.y);
...
export function printSpec(spec) {
  const lines = [];
  lines.push(
    `SPEC comp ${spec.comp} ${spec.compSize.width}x${spec.compSize.height} ${spec.orientation}`,
  );
  lines.push(
    `PALETTE ${spec.palette.map((c) => `${c.hex}(${Math.round(c.coverage * 100)}%)`).join(' ')}`,
  );
  lines.push(`BANDS ${spec.bands.map((b) => `${Math.round(b.y * 100)}%`).join(' ') || 'none'}`);
  for (const r of spec.regions) {
    const b = r.box;
    lines.push(
      `REGION ${r.id.padEnd(18)} ${r.kind.padEnd(8)} ${r.medium.padEnd(8)} box x${Math.round(b.x * 100)}% y${Math.round(b.y * 100)}% w${Math.round(b.w * 100)}% h${Math.round(b.h * 100)}% (${r.px.w}x${r.px.h}px, ${r.aspect}:1) palette ${r.palette
        .slice(0, 3)
        .map((c) => c.hex)
        .join(' ')}${r.plate ? ` plate ${r.plate}` : ''}${r.note ? `  # ${r.note}` : ''}`,
    );
  }
...
async function main() {
  ...
  if (flag('print')) {
    ...
    if (!spec) {
      console.error(
        `comp-spec: no spec at ${specPath}; run with --comp <png> --regions <json> first`,
      );
      process.exit(1);
    }
  ...
  if (arg('plate-prompt')) {
    const spec = loadSpec(specPath);
    if (!spec) {
      console.error(`comp-spec: no spec at ${specPath}`);
      process.exit(1);
    }
    const region = spec.regions.find((r) => r.id === arg('plate-prompt'));
    if (!region) {
      console.error(`comp-spec: no region ${arg('plate-prompt')}`);
      process.exit(1);
    }
  ...
  if (arg('crop')) {
    ...
    if (!region) {
      console.error(
        `comp-spec: no region ${arg('crop')}; ids: ${spec.regions.map((r) => r.id).join(', ')}`,
      );
      process.exit(1);
    }
    const comp = loadRaster(spec.comp).image;
    let c =
      region.medium === 'raster' && !flag('raw')
        ? plateReference(comp, spec, region)
        : crop(comp, region.px.x, region.px.y, region.px.w, region.px.h);
    ...
    fs.writeFileSync(
      out,
      encodePng(c, { text: { 'impeccable:crop-of': `${spec.comp}#${region.id}` } }),
    );
    console.log(
      `CROP ${out} (${c.width}x${c.height}) region ${region.id} of ${spec.comp}. Reference only: regenerate the plate from it, never ship it.`,
    );
    return;
  }
  ...
  let comp;
  try {
    comp = loadRaster(compPath).image;
  } catch (e) {
    console.error(`comp-spec: cannot read ${compPath}: ${e.message}`);
    process.exit(1);
  }
  ...
  if (flag('grid')) {
    ...
    console.log(
      `GRID ${GRID_PATH} (${comp.width}x${comp.height} comp; cells A0 top-left to J9 bottom-right)`,
    );
    console.log(
      `PALETTE ${paletteOf(comp)
        .map((c) => `${c.hex}(${Math.round(c.coverage * 100)}%)`)
        .join(' ')}`,
    );
    console.log(
      `BANDS ${
        horizontalBands(comp)
          .filter((b) => b.strength > 0.2)
          .map((b) => `${Math.round(b.y * 100)}%`)
          .join(' ') || 'none'
      }`,
    );
    console.log(
  ...
  if (arg('regions')) {
    try {
      regionsInput = JSON.parse(fs.readFileSync(arg('regions'), 'utf8'));
    } catch (e) {
      console.error(`comp-spec: cannot read regions ${arg('regions')}: ${e.message}`);
      process.exit(1);
    }
  } else if (flag('auto')) {
  ...
  let spec;
  try {
    spec = measureRegions(comp, regionsInput, compPath);
  } catch (e) {
    console.error(`comp-spec: ${e.message}`);
    process.exit(1);
  }
