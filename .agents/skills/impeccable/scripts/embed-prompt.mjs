...
function readPrompt(imagePath, buffer = fs.readFileSync(imagePath)) {
  ...
  let prompt =
    type === 'png' ? parsePng(buffer).prompt : type === 'jpeg' ? readJpegCom(buffer) : null;
...
function parsePng(buffer) {
  ...
  while (offset + 12 <= buffer.length) {
    ...
    chunks.push({
      ...
      type,
      promptChunk,
      bytes: buffer.subarray(offset, offset + 12 + length),
    });
    offset += 12 + length;
  }
