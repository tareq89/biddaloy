...
export function nextStepAfterChoice({ key, scope, cwd = process.cwd(), env = process.env } = {}) {
  ...
  const why =
    buildPath === 'comp'
      ? 'from .impeccable config'
      : imageGen
        ? 'default: image generation is available'
        : 'default: comp-led unless no image tool exists; if your harness truly has none and there is no OpenAI key, this is code-led and you say so in one line';
  if (scope === 'surface') {
...
export function sameMainModulePath(left, right, platform = process.platform) {
  ...
  const normalizeDriveLetter = (value) =>
    value.replace(/^([a-z]):/i, (_, drive) => `${drive.toUpperCase()}:`);
    ...
    if (chosenIdx !== -1 || kindIdx !== -1) {
      ...
      try {
        ...
        if (scopeIdx !== -1 && args[scopeIdx + 1] === 'direction') {
          ...
          wf(
            resolve(process.cwd(), '.impeccable', 'build', 'pending.json'),
            JSON.stringify({ scope: 'direction', at: new Date().toISOString() }, null, 2),
          );
        }
      } catch {
