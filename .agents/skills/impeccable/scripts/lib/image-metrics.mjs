...
export function toGray(img) {
  ...
  for (let i = 0, p = 0; i < g.length; i++, p += 4) {
    ...
    const r = img.data[p] * a + 255 * (1 - a),
      gg = img.data[p + 1] * a + 255 * (1 - a),
      b = img.data[p + 2] * a + 255 * (1 - a);
...
export function ssimShifted(a, b, dx, dy, win = 8) {
  const w = a.width - Math.abs(dx),
    h = a.height - Math.abs(dy);
...
export function colorHistogram(img, sampleStep = 2) {
  ...
  for (let y = 0; y < img.height; y += sampleStep) {
    for (let x = 0; x < img.width; x += sampleStep) {
      ...
      const key =
        ((img.data[p] >> 4) << 8) | ((img.data[p + 1] >> 4) << 4) | (img.data[p + 2] >> 4);
      bins[key]++;
      n++;
    }
  }
...
export function dominantColors(img, k = 6, sampleStep = 3) {
  ...
  for (const e of entries) {
    ...
    if (best && bestD < 14) {
      const tw = best.w + e.w;
      best.rgb = [
        (best.rgb[0] * best.w + r * e.w) / tw,
        (best.rgb[1] * best.w + g * e.w) / tw,
        (best.rgb[2] * best.w + b * e.w) / tw,
      ];
...
export function paletteMatch(compColors, buildColors) {
  ...
  for (const c of compColors) {
    ...
    s += c.coverage * Math.max(0, 1 - best / 25);
    wsum += c.coverage;
  }
...
export function detailScore(imgA, imgB, cols = 12, rows = 8) {
  const a = detailGrid(imgA, cols, rows),
    b = detailGrid(imgB, cols, rows);
  ...
  for (let i = 0; i < a.cells.length; i++) {
    ...
    if (ca > floor) {
      s += Math.min(cb / ca, ca / cb) * ca;
      w += ca;
    }
    if (cb > ca * 1.8 && cb > floor * 2) {
      added += 1;
    }
    addedW += 1;
  }
  ...
  const raw = w ? s / w : 1;
  return {
    score: Math.max(0, raw - 0.5 * addedFraction),
    rawScore: raw,
    addedFraction,
    comp: a,
    build: b,
...
export function diffMap(imgA, imgB, workWidth = 384) {
  const h = Math.max(8, Math.round((imgA.height / imgA.width) * workWidth));
  const a = resize(imgA, workWidth, h),
    b = resize(imgB, workWidth, h);
  const out = new Float32Array(workWidth * h);
  for (let i = 0, p = 0; i < out.length; i++, p += 4) {
    const dr = a.data[p] - b.data[p],
      dg = a.data[p + 1] - b.data[p + 1],
      db = a.data[p + 2] - b.data[p + 2];
...
export function horizontalBands(img, workWidth = 128, minGap = 0.02) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < workWidth; x++) {
      const p = (y * workWidth + x) * 4;
      r += s.data[p];
      g += s.data[p + 1];
      b += s.data[p + 2];
    }
    rowMean[y * 3] = r / workWidth;
    rowMean[y * 3 + 1] = g / workWidth;
    rowMean[y * 3 + 2] = b / workWidth;
  }
  const edges = [];
  for (let y = 1; y < h; y++) {
    const d = Math.hypot(
      rowMean[y * 3] - rowMean[(y - 1) * 3],
      rowMean[y * 3 + 1] - rowMean[(y - 1) * 3 + 1],
      rowMean[y * 3 + 2] - rowMean[(y - 1) * 3 + 2],
    );
