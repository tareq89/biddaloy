...
function otsu(gray) {
  ...
  for (let i = 0; i < gray.data.length; i++)
    hist[Math.max(0, Math.min(255, Math.round(gray.data[i])))]++;
...
function binarize(img) {
  ...
  let thr = otsu(g);
  let dark = 0;
  for (let i = 0; i < g.data.length; i++) if (g.data[i] < thr) dark++;
  ...
  const inkLevel = nI ? sI / nI : inkIsDark ? 0 : 255,
    groundLevel = nG ? sG / nG : inkIsDark ? 255 : 0;
  ...
  for (let i = 0; i < g.data.length; i++)
    covA[i] = Math.max(0, Math.min(1, (groundLevel - g.data[i]) / den));
  const cov = (i) => covA[i];
...
function findLines(bin) {
  ...
  for (let y = 0; y < H; y++) {
    const o = y * W;
    for (let x = 0; x < W; x++) colInk[x] += ink[o + x];
  }
  ...
  for (let y = 0; y < H; y++) {
    let c = 0;
    const o = y * W;
    for (let x = 0; x < W; x++) if (colOk[x]) c += ink[o + x];
    rowInk[y] = c;
  }
  ...
  while (y < H) {
    if (rowInk[y] > floor) {
      const y0 = y;
      while (y < H && (rowInk[y] > floor || (y + 1 < H && rowInk[y + 1] > floor))) y++;
  ...
  for (const run of runs) {
    let peak = 0;
    for (let yy = run.y0; yy < run.y1; yy++) peak = Math.max(peak, rowInk[yy]);
    ...
    for (let yy = run.y0; yy < run.y1; yy++) {
      ...
      if (!low && inValley) {
        inValley = false;
        if (yy - valleyStart >= 3 && valleyStart - start >= 4) {
          lines.push({ y0: start, y1: valleyStart, run });
          start = yy;
        }
      }
    }
  ...
  for (const ln of lines) {
    let m = 0;
    for (let yy = ln.y0; yy < ln.y1; yy++) m += rowInk[yy];
    ln.mass = m;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    const ln = lines[i];
    if (ln.tall) continue;
    if (ln.mass >= maxMass * 0.3) {
      merged.push({ y0: ln.y0, y1: ln.y1, mass: ln.mass });
      continue;
    }
    ...
    if (
      next &&
      next.run === ln.run &&
      next.mass >= maxMass * 0.3 &&
      ln.y1 - ln.y0 <= (next.y1 - next.y0) * 0.5
    ) {
      next.y0 = ln.y0;
    }
  }
...
export const FEATURES = [
  ...
  'densX',
  'runDensity',
  ...Array.from({ length: VBINS }, (_, i) => `vprof${i}`),
  ...HQ.map((q) => `hrun${Math.round(q * 100)}`),
...
function lineMetrics(bin, ln) {
  ...
  const dsc = cols
    .filter((c) => c.bot > baseF + tol * 1.5 && c.top < baseF - R * 0.3)
    .map((c) => (c.bot - baseF) / R);
...
function segment(bin, ln, m) {
  ...
  for (let yy = bandTop; yy < m.base; yy++) {
    const o = yy * W;
    for (let x = m.xL; x < m.xR; x++) colBand[x] += ink[o + x];
  }
  const runs = [];
  let x = m.xL;
  while (x < m.xR) {
...
function measure(bin, lines) {
  ...
  const hLen = (o, x0, x1) => {
    let s = 0;
    for (let x = Math.max(0, x0 - 1); x < Math.min(W, x1 + 1); x++) s += covA[o + x];
    return s;
  };
  const vLen = (x, y0, y1) => {
    let s = 0;
    for (let y = Math.max(0, y0 - 1); y < Math.min(H, y1 + 1); y++) s += covA[y * W + x];
    return s;
  };
  let glyphN = 0;
  const per = { xh: [], desc: [], runDensity: [] };
  let allCapsLines = 0;
  const vprof = new Float64Array(VBINS);
  const hruns = [],
    vruns = [],
    colHs = [],
    widths = [];
  ...
  for (const m of metrics) {
    ...
    for (let yy = Math.max(ln.y0, Math.round(base - cap)); yy < base; yy++) {
      ...
      while (x < xR) {
        if (ink[o + x]) {
          const x0 = x;
          while (x < xR && ink[o + x]) x++;
          hruns.push(hLen(o, x0, x) / cap);
        } else x++;
      }
    }
    for (let x = xL; x < xR; x++) {
      ...
      while (yy < ln.y1) {
        if (ink[yy * W + x]) {
          const y0 = yy;
          while (yy < ln.y1 && ink[yy * W + x]) yy++;
          vruns.push(vLen(x, y0, yy) / cap);
        } else yy++;
      }
    }
    ...
    const xs = xh
      ? onBase.filter((g) => Math.abs(base - g.top - xh) <= Math.max(tol * 1.5, cap * 0.05))
      : [];
    for (const g of capG) {
      advTall.push(g.w / cap);
      densTall.push(g.area / (g.w * g.h));
    }
    for (const g of xs) {
      densX.push(g.area / (g.w * g.h));
      advX.push(g.w / cap);
    }
    for (const g of onBase) {
      advAll.push(g.w / cap);
      widths.push(g.w / cap);
      round.push(g.w / (base - g.top) > 0.9 ? 1 : 0);
    }
    for (let i = 0; i + 1 < G.length; i++) {
      const gap = G[i + 1].x0 - G[i].x1;
      if (gap >= 0 && gap < cap * 0.6) gaps.push(gap / cap);
    }
    const xTop = base - (xh || cap * 0.55);
    const bandTop = Math.round(xTop + (base - xTop) * 0.2),
      bandBot = Math.round(base - (base - xTop) * 0.2);
    let runCount = 0,
      runRows = 0;
    ...
    for (let x = xL; x < xR; x++) {
      ...
      while (yy < ln.y1) {
        if (ink[yy * W + x]) {
          const y0 = yy;
          while (yy < ln.y1 && ink[yy * W + x]) yy++;
          const L = vLen(x, y0, yy);
          if (L < cap * 0.35) thins.push(L / cap);
        } else yy++;
      }
    }
    ...
    const runAt = (yy, x) => {
      ...
      let a = x,
        b = x;
      while (a > xL && ink[o + a - 1]) a--;
      while (b + 1 < xR && ink[o + b + 1]) b++;
      return hLen(o, a, b + 1);
    };
    const yMid = Math.round(base - cap * 0.4),
      yHi = Math.round(base - cap * 0.18),
      yFoot = base - Math.max(1, Math.round(cap * 0.04));
    let x = xL;
    while (x < xR) {
      let yy = base - 1;
      if (!ink[yy * W + x]) {
      ...
      const x0 = x;
      x++;
      while (x < xR && ink[(base - 1) * W + x] && ink[yMid * W + x]) x++;
      const xc = Math.round((x0 + x - 1) / 2);
      const wMid = runAt(yMid, xc),
        wHi = runAt(yHi, xc),
        wFoot = runAt(yFoot, xc);
      if (wMid > 0 && wMid < cap * 0.5 && wHi <= wMid * 1.3 && wHi >= wMid * 0.7)
        serifR.push(wFoot / wMid);
    }
  }
  if (!capN) return null;
  const stemW = med(stems),
    thinW = med(thins);
  const advM = med(advAll);
  const advSd =
    advAll.length > 3
      ? Math.sqrt(advAll.reduce((s, v) => s + (v - advM) ** 2, 0) / advAll.length)
      : null;
  const vsum = vprof.reduce((s, x) => s + x, 0) || 1;
  const extra = {};
  for (let i = 0; i < VBINS; i++) extra[`vprof${i}`] = vprof[i] / vsum;
  for (const q of HQ) {
    extra[`hrun${Math.round(q * 100)}`] = pct(hruns, q);
    extra[`vrun${Math.round(q * 100)}`] = pct(vruns, q);
  }
  extra.colq25 = pct(colHs, 0.25);
  extra.colq75 = pct(colHs, 0.75);
  extra.wq25 = pct(widths, 0.25);
  extra.wq75 = pct(widths, 0.75);
  return {
    ...extra,
...
export function isolateDominant(bin, lines, { tol = 0.28 } = {}) {
  ...
  for (const x of [...ms].sort((a, b) => b.m.cap - a.m.cap)) {
    ...
    if (c) {
      c.items.push(x);
      c.mass += x.ln.mass || 0;
    } else clusters.push({ cap: x.m.cap, items: [x], mass: x.ln.mass || 0 });
  }
  ...
  for (const c of clusters) {
    c.rows = c.items.reduce((n, x) => n + (x.ln.y1 - x.ln.y0), 0);
    c.density = c.mass / Math.max(1, c.rows);
    c.n = c.items.length;
  }
  clusters.sort((a, b) => {
    const aMulti = a.n >= 3,
  ...
  return {
    lines: keep.map((x) => x.ln),
    x0: Math.max(0, x0 - pad),
    x1: Math.min(W, x1 + pad),
    dropped: ms.length - keep.length,
  };
}
...
function maskOutside(bin, x0, x1, lines) {
  ...
  const ink2 = new Uint8Array(ink.length),
    cov2 = new Float32Array(covA.length);
  for (let y = 0; y < H; y++) {
...
export function fingerprint(img, { minCap = 24, minGlyphs = 3, isolate = true } = {}) {
  ...
  if (f.capHeightPx < minCap && f.capHeightPx >= 4) {
    ...
    if (isolate && lines.length > 1) {
      ...
      if (iso2 && (iso2.dropped > 0 || iso2.x1 - iso2.x0 < bin.W * 0.9)) {
        bin = maskOutside(bin, iso2.x0, iso2.x1, iso2.lines);
        lines = iso2.lines;
        isolated = Math.max(isolated, iso2.dropped);
      }
    }
  ...
  const r = {
    lines: lines.length,
    glyphs: f.glyphs,
    capHeightPx: +(f.capHeightPx / scale).toFixed(1),
    inkIsDark: bin.inkIsDark,
    upsampled: scale > 1,
    allCaps: f.allCaps,
    isolatedFrom: isolated,
    weight: f.densTall == null && f.densX == null ? null : +(f.densTall ?? f.densX).toFixed(4),
  };
...
export function grossGap(a, b) {
  const pick = (f, keys) => {
    for (const k of keys) if (f[k] != null) return { k, v: f[k] };
    return null;
  };
  const wa = pick(a, ['advX', 'advTall', 'advance']),
    wb = wa ? (b[wa.k] != null ? { k: wa.k, v: b[wa.k] } : null) : null;
  const ha = pick(a, ['densTall', 'densX', 'stemW']),
    hb = ha ? (b[ha.k] != null ? { k: ha.k, v: b[ha.k] } : null) : null;
...
export function _debugLines(img) {
  ...
  return lines.map((ln) => {
    ...
    if (!m) return { ln, m: null };
    const hs = m.hs.map((h) => +(h / m.R).toFixed(2)).sort((a, b) => a - b);
    const hist = {};
    for (const h of hs) {
      const b = Math.round(h * 20) / 20;
      hist[b] = (hist[b] || 0) + 1;
    }
    return {
      y0: ln.y0,
