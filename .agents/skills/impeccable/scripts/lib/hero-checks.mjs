...
export function textRegionCheck(region, compCrop, buildCrop, { capTol = 0.22, minCap = 10 } = {}) {
  ...
  // in five builds
  const colourOnly = () => {
    const ca = inkColor(compCrop),
      cb = inkColor(buildCrop);
    if (ca && cb && ca.ink && cb.ink && deltaE(ca.ink.lab, cb.ink.lab) > 22)
      findings.push(
        `text ${region.id}: ink is ${cb.ink.hex} in the build, ${ca.ink.hex} in the comp; use the comp's colour`,
      );
    return { findings, metrics: null };
  };
  if (!comp || !comp.capHeightPx || comp.capHeightPx < minCap || comp.glyphs < 6)
    return colourOnly();
  ...
  const metrics = {
    comp: { cap: comp.capHeightPx, lines: comp.lines, glyphs: comp.glyphs },
    build: bfp ? { cap: bfp.capHeightPx, lines: bfp.lines, glyphs: bfp.glyphs } : null,
  };
  if (!bfp || bfp.glyphs < 4) {
  ...
  if (comp.lines >= 2 && bfp.lines !== comp.lines && Math.abs(bfp.lines - comp.lines) >= 1) {
    ...
    const ba0 = inkBox(compCrop),
      bb0 = inkBox(buildCrop);
    if (ba0 && bb0) {
      const pa = ba0.h / comp.lines,
        pb = bb0.h / bfp.lines;
      const dp = (pb - pa) / pa;
      if (Math.abs(dp) > 0.2)
        findings.push(
          `text ${region.id}: line pitch ${Math.round(pb)}px in the build, ${Math.round(pa)}px in the comp (${dp > 0 ? '+' : ''}${Math.round(dp * 100)}%); set line-height so ${comp.lines} lines stand ${Math.round(ba0.h)}px tall`,
        );
    }
  }
  ...
  if (
    comp.gap != null &&
    bfp.gap != null &&
    Math.abs(capDelta) <= capTol &&
    comp.glyphs >= 8 &&
    bfp.glyphs >= 8
  ) {
    const dg = bfp.gap - comp.gap;
    if (Math.abs(dg) > Math.max(0.03, comp.gap * 0.5))
      findings.push(
        `text ${region.id}: letter-spacing is ${dg > 0 ? 'wider' : 'tighter'} than the comp's (gap ${bfp.gap.toFixed(3)} vs ${comp.gap.toFixed(3)} of the cap height); set letter-spacing to ${dg > 0 ? 'close' : 'open'} it by about ${Math.abs(Math.round(dg * comp.capHeightPx))}px`,
      );
  }
  ...
  if (comp.densTall != null && bfp.densTall != null && Math.abs(capDelta) <= capTol) {
    ...
    if (r > 1.25)
      findings.push(
        `text ${region.id}: the face renders ${Math.round((r - 1) * 100)}% heavier than the comp's (ink density ${bfp.densTall.toFixed(2)} vs ${comp.densTall.toFixed(2)}); drop a weight step or use the ranked face`,
      );
    else if (r < 0.75)
      findings.push(
        `text ${region.id}: the face renders ${Math.round((1 - r) * 100)}% lighter than the comp's (ink density ${bfp.densTall.toFixed(2)} vs ${comp.densTall.toFixed(2)}); raise a weight step or use the ranked face`,
      );
  }
  ...
  const ba = inkBox(compCrop),
    bb = inkBox(buildCrop);
  if (ba && bb) {
    const dy = bb.y - ba.y;
    if (Math.abs(dy) > Math.max(12, compCrop.height * 0.15))
      findings.push(
        `text ${region.id}: its first line starts ${Math.abs(Math.round(dy))}px ${dy > 0 ? 'lower' : 'higher'} than in the comp (${bb.y}px vs ${ba.y}px into the region box); the spacing above it is ${dy > 0 ? 'too large' : 'too small'}`,
      );
    const dx = bb.x - ba.x;
    if (Math.abs(dx) > Math.max(12, compCrop.width * 0.15))
      findings.push(
        `text ${region.id}: it starts ${Math.abs(Math.round(dx))}px ${dx > 0 ? 'further right' : 'further left'} than in the comp`,
      );
  }
...
export function chromeStripCheck(region, compCrop, buildCrop) {
  ...
  const ra = ruleRows(compCrop),
    rb = ruleRows(buildCrop);
  if (ra.length && rb.length) {
    ...
    const ya = ra[0],
      yb = rb[0];
    const dy = yb - ya;
    if (Math.abs(dy) > Math.max(5, compCrop.height * 0.06))
      findings.push(
        `${region.kind} ${region.id}: its rule sits ${ya}px into the box in the comp and ${yb}px in the build (${dy > 0 ? '+' : ''}${dy}px), so the strip is ${dy > 0 ? 'taller' : 'shorter'} than the comp's; match the height, not only the position`,
      );
  ...
  if (ba.w >= compCrop.width * 0.6 && ba.h <= compCrop.height * 0.6) {
    const dh = bb.h - ba.h;
    if (Math.abs(dh) > Math.max(10, ba.h * 0.25))
      findings.push(
        `${region.kind} ${region.id}: its ink is ${bb.h}px tall in the build and ${ba.h}px in the comp (${dh > 0 ? '+' : ''}${dh}px); match the height, not only the position`,
      );
  }
 ...
 */
export function inventedInk(
  comp,
  build,
  { cols = 10, rows = 10, floor = 10, added = 12, ratio = 2.5 } = {},
) {
  const a = detailGrid(comp, cols, rows, 512),
    b = detailGrid(build, cols, rows, 512);
  const cells = [];
    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let dr = -1; dr <= 1; dr++)
        for (let dc = -1; dc <= 1; dc++) {
          ...
          neighbourhood += a.cells[rr * cols + cc];
          n++;
        }
      ...
      cells.push({
        col: c,
        row: r,
        label: `${String.fromCharCode(65 + c)}${r}`,
        comp: +a.cells[i].toFixed(1),
        build: +b.cells[i].toFixed(1),
      });
    }
...
export function plateClipCheck(region, compCrop, buildCrop, { margin = 6 } = {}) {
  const a = inkBox(compCrop),
    b = inkBox(buildCrop);
  if (!a || !b) return { sides: [] };
  const W = compCrop.width,
    H = compCrop.height;
  const sides = [];
  const flush = (v) => v <= 1;
...
export function svgIllustrations(html, { iconPx = 64, pathBudget = 480, maxPaths = 8 } = {}) {
  ...
  while ((m = re.exec(html))) {
    const attrs = m[1],
      body = m[2];
    const paths =
      (body.match(/<path\b/gi) || []).length +
      (body.match(/<(polyline|polygon|line|circle|ellipse|rect)\b/gi) || []).length;
    let budget = 0;
    ...
    const w = /\swidth="([\d.]+)(px)?"/.exec(attrs),
      h = /\sheight="([\d.]+)(px)?"/.exec(attrs);
    ...
    if (budget <= pathBudget && paths <= maxPaths && long === 0 && !/<(text|image)\b/i.test(body))
      continue; // a tiny inline glyph with no size hint
    if (budget > pathBudget || paths > maxPaths || (long > iconPx && paths > 0)) {
      const id = /\b(id|class|aria-label|data-region)="([^"]+)"/i.exec(attrs);
      out.push({
        snippet: `<svg${attrs.slice(0, 80).replace(/\s+/g, ' ')}...> (${paths} shapes, ${budget} chars of path data${long ? `, ${long}px` : ''})`,
        label: id ? id[2] : null,
        paths,
        budget,
        long,
