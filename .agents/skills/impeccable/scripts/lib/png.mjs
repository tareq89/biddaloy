  ...
  for (let y = 0; y < height; y++) {
    ...
    switch (filter) {
      ...
      case 1:
        ...
        break;
      case 2:
        if (prev) for (let i = 0; i < stride; i++) line[i] = (line[i] + prev[i]) & 0xff;
        break;
      case 3:
...
export function decodePng(buf) {
  ...
  if (trnsChunk) {
    if (colorType === 3) trns = { alpha: trnsChunk.data };
      ...
      trns = { gray: trnsChunk.data.readUInt16BE(0) >> (bitDepth === 16 ? 8 : 0) };
  }
  ...
  if (interlace === 0) {
    ...
    for (const [sx, sy, dx, dy] of passes) {
      ...
      const { bytes, stride, consumed } = unfilter(
        raw.subarray(offset),
        pw,
        ph,
        bpp,
...
export function encodePng({ width, height, data }, { text = null, level = 6 } = {}) {
  if (data.length !== width * height * 4)
    throw new Error(`png: data length ${data.length} != ${width}x${height}x4`);
...
export function loadRaster(file) {
  ...
  throw new Error(
    `png: ${file} is not a PNG and no converter (dwebp, sips, magick, convert) could produce ${cache}${lastErr ? `: ${lastErr.message}` : ''}`,
  );
}
