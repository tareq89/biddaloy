...
export function resize(img, width, height) {
  ...
  if (width === img.width && height === img.height)
    return { width, height, data: new Uint8Array(img.data) };
  ...
  if (sx >= 1 && sy >= 1) {
    for (let y = 0; y < height; y++) {
      const y0 = Math.floor(y * sy),
        y1 = Math.min(img.height, Math.max(y0 + 1, Math.floor((y + 1) * sy)));
      for (let x = 0; x < width; x++) {
        const x0 = Math.floor(x * sx),
          x1 = Math.min(img.width, Math.max(x0 + 1, Math.floor((x + 1) * sx)));
        let r = 0,
          g = 0,
          b = 0,
          a = 0,
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let c = 0; c < 4; c++) {
        const p00 = img.data[(y0 * img.width + x0) * 4 + c],
          p10 = img.data[(y0 * img.width + x1) * 4 + c];
        const p01 = img.data[(y1 * img.width + x0) * 4 + c],
          p11 = img.data[(y1 * img.width + x1) * 4 + c];
  ...
  for (let yy = 0; yy < src.height; yy++) {
    ...
    for (let xx = 0; xx < src.width; xx++) {
      ...
      const da = dst.data[d + 3] / 255,
        oa = a + da * (1 - a);
      for (let c = 0; c < 3; c++)
        dst.data[d + c] = (src.data[s + c] * a + dst.data[d + c] * da * (1 - a)) / (oa || 1);
      dst.data[d + 3] = oa * 255;
    }
  }
}
  ...
  for (let yy = r.y; yy < r.y + r.h; yy++) {
    for (let xx = r.x; xx < r.x + r.w; xx++) {
      ...
      if (a >= 1) {
        ...
        for (let c = 0; c < 3; c++) img.data[o + c] = rgba[c] * a + img.data[o + c] * (1 - a);
        img.data[o + 3] = Math.max(img.data[o + 3], a * 255);
      }
    }
  }
}
...
export function drawText(img, text, x, y, rgba, scale = 2) {
  ...
  for (const chRaw of String(text).toUpperCase()) {
    ...
    for (let r = 0; r < 7; r++)
      for (let c = 0; c < 5; c++)
        if (g[r][c] === '1') fillRect(img, cx + c * scale, y + r * scale, scale, scale, rgba);
    cx += 6 * scale;
  }
  return cx - x;
}
...
export function drawLabel(
  ...
  x,
  y,
  { fg = [255, 255, 255, 255], bg = [0, 0, 0, 220], scale = 2, pad = 4 } = {},
) {
  const w = textWidth(text, scale) + pad * 2,
    h = 7 * scale + pad * 2;
