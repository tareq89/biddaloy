...
if (plateId) {
  ...
  if (!spec) {
    console.error(`generate-image: no spec at ${specPath}; run comp-spec.mjs first`);
    process.exit(1);
  }
  const region = spec.regions.find((r) => r.id === plateId);
  if (!region) {
    console.error(
      `generate-image: no region ${plateId} in ${specPath}; ids: ${spec.regions.map((r) => r.id).join(', ')}`,
    );
    process.exit(1);
  }
  if (region.medium !== 'raster') {
    console.error(
      `generate-image: region ${plateId} is ${region.medium}, not a plate; set its kind to plate|image|texture in the regions file`,
    );
    process.exit(1);
  }
  let comp;
  try {
    ...
    console.error(`generate-image: cannot read comp ${spec.comp}: ${e.message}`);
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    refPath,
    encodePng(ref, { text: { 'impeccable:crop-of': `${spec.comp}#${region.id}` } }),
  );
  ...
  const extra =
    arg('prompt') || (arg('prompt-file') ? fs.readFileSync(arg('prompt-file'), 'utf8') : '');
  ...
  const wantsChroma = process.argv.includes('--chroma')
    ? true
    : process.argv.includes('--no-chroma')
      ? false
      : region.kind === 'plate' && inkOnGround(region);
  const chromaColor = '#00ff00';
  const chromaLine = wantsChroma
    ? ` Render the artwork on a perfectly flat, uniform bright green background (${chromaColor}) that fills every pixel not covered by the artwork; no paper texture, no vignette, no shadow on the green; the green will be removed and the artwork composited onto the page's own surface.`
    : '';
  ...
  plateCtx = {
    ...
    encodePng,
    resize,
    chroma: wantsChroma ? chromaColor : null,
  };
  if (process.env.IMPECCABLE_IMAGE_GEN_FAKE) {
    const up = resize(ref, ref.width * 2, ref.height * 2);
    fs.writeFileSync(
      out,
      encodePng(up, { text: { 'impeccable:prompt': prompt, 'impeccable:fake': '1' } }),
    );
    fs.writeFileSync(
      `${out}.json`,
      JSON.stringify(
    ...
    console.log(
      `PLATE: ${out} (${up.width}x${up.height}, fake 2x crop of region ${region.id}, $0.00, no API call)`,
    );
    process.exit(0);
  }
...
async function keyChroma(file, keyHex) {
  ...
  for (let i = 0; i < img.data.length; i += 4) {
    ...
    if (d < soft) {
      ...
      const m = (r + b) / 2;
      img.data[i + 1] = Math.round(g * a + m * (1 - a));
    }
  }
  // keep the tEXt chunks (the embedded prompt written before keying)
  fs.writeFileSync(
    file,
    encodePng(img, { text: img.text && Object.keys(img.text).length ? img.text : null }),
  );
...
async function scorePlate(ctx, outFile) {
  try {
    ...
    if (ctx.chroma) {
      ...
      const g =
        (ctx.region.palette && ctx.region.palette[0] && ctx.region.palette[0].hex) || '#ffffff';
    ...
    if (!v.ok)
      console.log(
        `PLATE-WARN the plate does not read as region ${ctx.region.id}: ${v.reasons.join('; ')}. Open ${outFile} beside ${ctx.refPath} and regenerate before building on it; the plates gate refuses it as it stands.`,
      );
    if (min != null && s.overall < min) {
      console.log(`PLATE-REJECTED below --min ${(min * 100).toFixed(0)}%`);
      process.exit(3);
    }
  } catch (e) {
...
// it. Refuse mock output until start has run (or --force-mock).
{
  ...
  const intoMocks =
    /(^|[\\/])\.impeccable[\\/]mocks[\\/]/.test(outArg) && !/[\\/]decision[\\/]/.test(outArg);
...
try {
  ...
  const result = spawnSync(
    process.execPath,
    [fileURLToPath(new URL('./embed-prompt.mjs', import.meta.url)), out, '--prompt', prompt],
    { stdio: 'ignore' },
  );
...
console.log(
  `IMAGE: ${out} (${size}, ${quality}, gpt-image-2, billed to your OpenAI key); ${embedded ? 'prompt embedded + sidecar' : 'sidecar'} at ${out}.json`,
);
if (plateCtx && plateCtx.chroma) {
  const frac = await keyChroma(out, plateCtx.chroma);
  console.log(
    `PLATE-CHROMA keyed ${(frac * 100).toFixed(0)}% of pixels to alpha (${plateCtx.chroma}); place with a plain <img> over the page's own ground, no background on the plate. If the keyed fraction is under 20% the generator ignored the key: regenerate with --no-chroma and use mix-blend-mode: multiply instead.`,
  );
}
