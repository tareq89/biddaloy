...
describe('notification-state', () => {
  ...
  it('caps history at 50, dropping the oldest', () => {
    for (let i = 0; i < 55; i++) {
      pushNotification({ message: `Notification ${i}`, variant: 'info' });
    }
  ...
  it('markAllNotificationsRead clears every unread flag', () => {
    pushNotification({ message: 'First', variant: 'info' });
    pushNotification({ message: 'Second', variant: 'error' });
  ...
  it('a tenant switch clears history so one school cannot see another’s notifications', () => {
    ...
    pushNotification({ message: "Tenant A's import finished", variant: 'success' });
  ...
  it('logout (clearAuthState) also clears history', () => {
    setActiveTenant('tenant-a');
    pushNotification({ message: 'Something happened', variant: 'info' });
    expect(getNotifications()).toHaveLength(1);
