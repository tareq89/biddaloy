...
### [18.3.2] Single-runner tuning options considered

CI's frontend job runs on a 4 vCPU runner; a 604s work / 434s wall ratio
(~1.4x parallelism) motivated re-checking every single-runner lever before
falling back to sharding ([18.5.1]). This worktree has no push access to
...
|---|---|---|
| a. `maxWorkers: 4` on every project | **kept, partial fix** | Left unset, Vitest sizes each of the six `projects` entries' pool independently off `os.availableParallelism()` (falling back to `os.cpus().length`), and has been observed sizing pools differently under `--coverage`. Pinning it to 4 makes every project's pool size explicit and environment-independent — but it does **not** cap the total worker count across the run: six projects × 4 worker threads each is still up to 24 worker threads queued onto 4 cores, since `projects` mode has no shared, cross-project worker budget to pin instead. The 604s/434s (~1.4x) ratio this ticket set out to explain is likely this `projects`-mode architecture limit, which this option does not resolve — flagged as the residual for [18.5.1]'s sharding call, not re-measured against real CI (see above). |
| b. jsdom `isolate: false` + `afterEach` timer/module cleanup in `ui/src/test/setup.ts` | **skipped** | `:jsdom` projects already run with Vitest's default `isolate: true` — each test **file** gets a fresh module registry, so there is no cross-file timer/module leakage for a global `afterEach(vi.useRealTimers)`/`afterEach(vi.resetModules)` to guard against today; adding it would be a no-op. Turning `isolate: false` on for `:jsdom` itself stays off: `vitest.config.ts`'s own `NODE_PROJECT_ISOLATE` comment documents a live hazard blocking it — a partial `vi.mock('@biddaloy/ui/api')` in `client-admin/src/main.sentry-wiring.test.ts` (omits `ensureSessionLoaded`) that would poison `select-school.test.tsx` and `_staff/invoices/$invoiceId.test.tsx` if they shared its worker. Fixing that mock is real work across multiple files and outside this ticket's `## Files` (`vitest.config.ts`, `ui/src/test/setup.ts`, this README) — flagged here rather than edited silently, same as the comment already says: not yet filed as its own issue under epic #428. |
| c. `deps.optimizer.web.include: ['@biddaloy/ui']` | **skipped** | Vite's dep optimizer pre-bundles real `node_modules` dependencies. `@biddaloy/ui` isn't resolved that way here — `vitest.config.ts`'s `clientAlias()` maps it straight to `ui/src` via `resolve.alias`, so it's already treated as first-party source transformed per file, not a package the optimizer would ever see. The option has nothing to attach to. |
| d. `coverage.experimentalAstAwareRemapping: true` | **skipped** | Not a real option on the installed toolchain — `@vitest/coverage-v8@4.1.11`'s `CoverageOptions`/provider types (checked in `node_modules/@vitest/coverage-v8/dist/provider.d.ts`) have no such field, and setting it would be silently ignored (or a type error, depending on how it's spelled) rather than doing anything. |
| e. `pool: 'forks'` for `:jsdom` only | **skipped** | Contradicts the measurement already in the table above this section: `threads` beat `forks` on the combined suite (~61.7s vs ~69.4s wall, coverage off), and `:jsdom` is 153 of 201 files — the project already dominating that comparison. No reason to expect the opposite result on a subset of the same files. |
